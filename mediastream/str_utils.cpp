...
void datab_ref(dblk_t *d){
	d->db_ref++;
}

void datab_unref(dblk_t *d){
	d->db_ref--;
	if (d->db_ref==0){
		if (d->db_freefn!=NULL)
...
mblk_t * appendb(mblk_t *mp, const char *data, int size, bool pad){
	int padcnt=0;
	int i;
	if (pad){
...
void msgappend(mblk_t *mp, const char *data, int size, bool pad){
...
void ms_bufferizer_put_from_queue(MSBufferizer *obj, queue_t *q){
	mblk_t *m;
	while((m=getq(q))!=NULL){
