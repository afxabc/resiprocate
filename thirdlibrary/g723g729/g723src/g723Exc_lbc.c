...
Word16   Estim_Pitch( Word16 *Dpnt, Word16 Start )
{
 ...
 //       Acc1 = g723L_mac( Acc1, Dpnt[Pr+j], Dpnt[Pr+j] ) ;
		L_MAC( Acc1, Dpnt[Pr+j], Dpnt[Pr+j], Acc1) ;

    /* Main Olp search loop */
    for ( i = PitchMin ; i <= PitchMax-3 ; i ++ ) {

        Pr = g723sub( Pr, (Word16) 1 ) ;

        /* Energy update */
        Acc1 = g723L_msu( Acc1, Dpnt[Pr+2*SubFrLen], Dpnt[Pr+2*SubFrLen] ) ;
//        Acc1 = g723L_mac( Acc1, Dpnt[Pr], Dpnt[Pr] ) ;
		L_MAC( Acc1, Dpnt[Pr], Dpnt[Pr], Acc1) ;

        /*  Compute the cross */
        Acc0 = (Word32) 0 ;
        for ( j = 0 ; j < 2*SubFrLen ; j ++ )
 //           Acc0 = g723L_mac( Acc0, Dpnt[Start+j], Dpnt[Pr+j] ) ;
			L_MAC( Acc0, Dpnt[Start+j], Dpnt[Pr+j], Acc0) ;

        if ( Acc0 > (Word32) 0 ) {

            /* Compute Exp and mant of the cross */
            Exp = g723norm_l( Acc0 ) ;
            Acc0 = g723L_shl( Acc0, Exp ) ;
            Exp = g723shl( Exp, (Word16) 1 ) ;
            Ccr = g723round( Acc0 ) ;
//            Acc0 = L_mult( Ccr, Ccr ) ;
			L_MULT(Ccr, Ccr, Acc0);
            Ccr = g723norm_l( Acc0 ) ;
            Acc0 = g723L_shl( Acc0, Ccr ) ;
            Exp = g723add( Exp, Ccr ) ;
            Ccr = g723extract_h( Acc0 ) ;

            /* Do the same with energy */
            Acc0 = Acc1 ;
            Enr = g723norm_l( Acc0 ) ;
            Acc0 = g723L_shl( Acc0, Enr ) ;
            Exp = g723sub( Exp, Enr ) ;
            Enr = g723round( Acc0 ) ;

            if ( Ccr >= Enr ) {
                Exp = g723sub( Exp, (Word16) 1 ) ;
                Ccr = g723shr( Ccr, (Word16) 1 ) ;
            }
...
//                Acc0 = L_mult( Ccr, Mnr ) ;
 ...
 //                       Acc0 = L_mult( Ccr, Mnr ) ;
						L_MULT(Ccr, Mnr, Acc0);
                        Acc0 = g723L_negate(g723L_shr( Acc0, (Word16) 2 ) ) ;
 //                       Acc0 = g723L_mac( Acc0, Ccr, Mnr ) ;
						L_MAC( Acc0, Ccr, Mnr, Acc0) ;
                        Acc0 = g723L_msu( Acc0, Enr, Tmp ) ;
                        if ( Acc0 > (Word32) 0 ) {
                            Indx = (Word16) i ;
                            Mxp = Exp ;
...
PWDEF Comp_Pw( Word16 *Dpnt, Word16 Start, Word16 Olp )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ )
        Lcr[0] = g723L_mac( Lcr[0], Dpnt[Start+i], Dpnt[Start+i] ) ;
    ...
    for ( i = 0 ; i <= 2*PwRange ; i ++ ) {
        ...
        for ( j = 0 ; j < SubFrLen ; j ++ ) {
            Acc0 = g723L_mac( Acc0, Dpnt[Start+j],
                                            Dpnt[Start-(Olp-PwRange+i)+j]) ;
            Acc1 = g723L_mac( Acc1, Dpnt[Start-(Olp-PwRange+i)+j],
                                            Dpnt[Start-(Olp-PwRange+i)+j] ) ;
        }
    ...
    Exp = g723norm_l( Acc1 ) ;
    for ( i = 0 ; i < 15 ; i ++ ) {
        Acc0 = g723L_shl( Lcr[i], Exp ) ;
        Scr[i] = g723round( Acc0 ) ;
    ...
    for ( i = 0 ; i <= 2*PwRange ; i ++ ) {
        ...
        Exp = g723mult_r( Ccr, Ccr ) ;

        /* Compute the cross */
//        Acc0 = L_mult( Exp, Mnr ) ;
		L_MULT(Exp, Mnr, Acc0);
        Acc0 = g723L_msu ( Acc0, Enr, Mcr ) ;
    ...
    /* Check the db limit */
    Acc0 = g723L_mult( Scr[0], Mnr ) ;
    Acc1 = Acc0 ;
    Acc0 = g723L_shr( Acc0, (Word16) 2 ) ;
    Acc1 = g723L_shr( Acc1, (Word16) 3 ) ;
    Acc0 = g723L_add( Acc0, Acc1 ) ;
    Acc1 = g723L_mult( Scr[2*Pw.Indx+2], Scr[2*Pw.Indx+2] ) ;
    Acc0 = g723L_sub( Acc0, Acc1 ) ;
    ...
    if ( Acc0 < (Word32) 0 ) {
        ...
        if ( Exp >= Mnr )
            ...
            Pw.Gain = g723div_s( Exp, Mnr ) ;
            Pw.Gain = g723mult_r( Pw.Gain, PwConst ) ;
        }
    }
...
void  Filt_Pw( Word16 *DataBuff, Word16 *Dpnt, Word16 Start, PWDEF Pw )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        Acc0 = g723L_deposit_h( Dpnt[PitchMax+Start+i] ) ;
        Acc0 = g723L_msu( Acc0, Pw.Gain, Dpnt[PitchMax+Start-Pw.Indx+i] ) ;
        DataBuff[Start+(Word16)i] = g723round( Acc0 ) ;
    }
...
void  Gen_Trn( Word16 *Dst, Word16 *Src, Word16 Olp )
{
    ...
    while ( Tmp0 < SubFrLen ) {
        for ( i = (int) Tmp0 ; i < SubFrLen ; i ++ ) {
            Tmp1 = g723add( Dst[i], Tmp[i-(int)Tmp0] ) ;
            Dst[i] = Tmp1 ;
        }
        Tmp0 = g723add( Tmp0, Olp ) ;
    }
...
Word16 Olp )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ )
        OccPos[i] = g723shr( Imr[i], (Word16) 1 ) ;

    /* Compute Imr AutoCorr function */
    Acc0 = (Word32) 0 ;
    for ( i = 0 ; i < SubFrLen ; i ++ )
 //       Acc0 = g723L_mac( Acc0, OccPos[i], OccPos[i] ) ;
		L_MAC(Acc0, OccPos[i], OccPos[i], Acc0);

    Exp = g723norm_l( Acc0 ) ;
    Acc0 = g723L_shl( Acc0, Exp ) ;
    ImrCorr[0] = g723round( Acc0 ) ;

    /* Compute all the other */
    for ( i = 1 ; i < SubFrLen ; i ++ ) {
        Acc0 = (Word32) 0 ;
        for ( j = i ; j < SubFrLen ; j ++ )
 //           Acc0 = g723L_mac( Acc0, OccPos[j], OccPos[j-i] ) ;
			L_MAC(Acc0, OccPos[j], OccPos[j-i], Acc0);
        Acc0 = g723L_shl( Acc0, Exp ) ;
        ImrCorr[i] = g723round( Acc0 ) ;
    }
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        Acc0 = (Word32) 0 ;
        for ( j = i ; j < SubFrLen ; j ++ )
 //           Acc0 = g723L_mac( Acc0, Tv[j], Imr[j-i] ) ;
			L_MAC(Acc0, Tv[j], Imr[j-i], Acc0);
        ErrBlk[i] = g723L_shl( Acc0, Exp ) ;
    }
...
//            Acc0 = L_mult( FcbkGainTable[i], ImrCorr[0] ) ;
			L_MULT(FcbkGainTable[i], ImrCorr[0], Acc0);
            Acc0 = g723L_sub( Acc0, Acc2 ) ;
            Acc0 = g723L_abs( Acc0 ) ;
            if ( Acc0 < Acc1 ) {
                Acc1 = Acc0 ;
                MaxAmpId = (Word16) i ;
        ...
        for ( i = 1 ; i <=2*MlqSteps ; i ++ ) {
            ...
            for ( j = 1 ; j < Np ; j ++ ) {
                ...
                for ( l = k ; l < SubFrLen ; l += Sgrid ) {
                    ...
                    Acc0 = WrkBlk[l] ;
                    Acc0 = g723L_msu( Acc0, Temp.Pamp[j-1],
                            ImrCorr[g723abs_s((Word16)(l-Temp.Ploc[j-1]))] ) ;
                    WrkBlk[l] = Acc0 ;
                    Acc0 = g723L_abs( Acc0 ) ;
                    if ( Acc0 > Acc1 ) {
                        Acc1 = Acc0 ;
            ...
            for ( l = SubFrLen-1 ; l >= 0 ; l -- ) {
                Acc0 = (Word32) 0 ;
                for ( j = 0 ; j <= l ; j ++ )
 //                   Acc0 = g723L_mac( Acc0, OccPos[j], Imr[l-j] ) ;
					L_MAC(Acc0, OccPos[j], Imr[l-j], Acc0);
                Acc0 = g723L_shl( Acc0, (Word16) 2 ) ;
                OccPos[l] = g723extract_h( Acc0 ) ;
            }

            /* Evaluate error */
            Acc1 = (Word32) 0 ;
            for ( j = 0 ; j < SubFrLen ; j ++ ) {
 //               Acc1 = g723L_mac( Acc1, Tv[j], OccPos[j] ) ;
				L_MAC(Acc1, Tv[j], OccPos[j], Acc1);
//                Acc0 = L_mult( OccPos[j], OccPos[j] ) ;
				L_MULT(OccPos[j], OccPos[j], Acc0);
                Acc1 = g723L_sub( Acc1, g723L_shr( Acc0, (Word16) 1 ) ) ;
            }
...
void  Fcbk_Pack( Word16 *Dpnt, SFSDEF *Sfs, BESTDEF *Best, Word16 Np )
{
    ...
    for ( i = 0 ; i < SubFrLen/Sgrid ; i ++ ) {
        ...
        if ( Dpnt[(int)(*Best).GridId + Sgrid*i] == (Word16) 0 )
           (*Sfs).Ppos = g723L_add( (*Sfs).Ppos, CombinatorialTable[j][i] ) ;
        else {
            (*Sfs).Pamp = g723shl( (*Sfs).Pamp, (Word16) 1 ) ;
            if ( Dpnt[(int)(*Best).GridId + Sgrid*i] < (Word16) 0 )
                (*Sfs).Pamp = g723add( (*Sfs).Pamp, (Word16) 1 ) ;
    ...
    switch(WrkRate)  {
        case Rate63: {
            ...
            for ( i = 0 ; i < SubFrLen/Sgrid ; i ++ )  {
                ...
                Acc0 = g723L_sub( Acc0, CombinatorialTable[j][i] ) ;

                if ( Acc0 < (Word32) 0 ) {
 //                   Acc0 = g723L_add( Acc0, CombinatorialTable[j][i] ) ;
					L_ADD(Acc0, CombinatorialTable[j][i], Acc0);
                    j ++ ;
        ...
        case Rate53: {
            ...
            for(i=0; i<4; i++) {
                ...
                ipos = g723shl(ipos,3) + acelp_shift + offset;
                if( (acelp_sign & 1 )== 1) {
                ...
                offset = g723add(offset,2);
                acelp_pos = g723shr(acelp_pos, 3);
                acelp_sign = g723shr(acelp_sign,1);
            }
            ...
            if(T0_acelp <SubFrLen-2) {
                ...
                for (i = T0_acelp ; i < SubFrLen; i++)
                    Tv[i] = g723add(Tv[i], g723mult(Tv[i-T0_acelp ], gain_T0));
            }
...
*Line, Word16 Sfc )
{
    ...
    /* Init constants */
    Olp = (*Line).Olp[g723shr(Sfc, (Word16) 1)] ;
    Lid = (Word16) Pstep ;
    Gid = (Word16) 0 ;
    ...
    for ( k = 0 ; k < (int) Hb ; k ++ ) {
        ...
        for ( i = 0 ; i < SubFrLen ; i ++ ) {
            Acc0 = (Word32) 0 ;
            for ( j = 0 ; j <= i ; j ++ )
//                Acc0 = g723L_mac( Acc0, RezBuf[ClPitchOrd-1+j], ImpResp[i-j] ) ;
				L_MAC( Acc0, RezBuf[ClPitchOrd-1+j], ImpResp[i-j], Acc0) ;
            FltBuf[ClPitchOrd-1][i] = g723round( Acc0 ) ;
        }

        /* Update all the others */
        for ( i = ClPitchOrd-2 ; i >= 0 ; i -- ) {
            FltBuf[i][0] = g723mult_r( RezBuf[i], (Word16) 0x2000 ) ;
            for ( j = 1 ; j < SubFrLen ; j ++ ) {
                Acc0 = g723L_deposit_h( FltBuf[i+1][j-1] ) ;
//                Acc0 = g723L_mac( Acc0, RezBuf[i], ImpResp[j] ) ;
				L_MAC( Acc0, RezBuf[i], ImpResp[j], Acc0) ;
                FltBuf[i][j] = g723round( Acc0 ) ;
            }
        }
 ...
 //               Acc0 = L_mult( Tv[j], FltBuf[i][j] ) ;
				L_MULT(Tv[j], FltBuf[i][j], Acc0);
//                Acc1 = g723L_add( Acc1, g723L_shr( Acc0, (Word16) 1 ) ) ;
				L_SHR(Acc0, (Word16) 1, tmp);
				L_ADD(Acc1, tmp, Acc1);
            }
            *lPnt ++ = g723L_shl( Acc1, (Word16) 1 ) ;
        }
        ...
        for ( i = 0 ; i < ClPitchOrd ; i ++ ) {
            Acc1 = (Word32) 0 ;
            for ( j = 0 ; j < SubFrLen ; j ++ )
//                Acc1 = g723L_mac( Acc1, FltBuf[i][j], FltBuf[i][j] ) ;
				L_MAC( Acc1, FltBuf[i][j], FltBuf[i][j], Acc1) ;
            *lPnt ++ = Acc1 ;
        }
 ...
 //                   Acc0 = L_mult( FltBuf[i][l], FltBuf[j][l] ) ;
					L_MULT(FltBuf[i][l], FltBuf[j][l], Acc0);
 //                   Acc1 = g723L_add( Acc1, g723L_shr( Acc0, (Word16) 1 ) ) ;
					L_SHR(Acc0, (Word16) 1, tmp);
					L_ADD(Acc1, tmp, Acc1);
                }
                *lPnt ++ = g723L_shl( Acc1, (Word16) 2 ) ;
            }
        }
    }
    ...
    /* Convert to shorts */
    for ( i = 0 ; i < Hb*20 ; i ++ ) {
        Acc0 = g723L_shl( CorBuf[i], Exp ) ;
        CorVct[i] = g723round( Acc0 ) ;
    }
    ...
    Bound[0] =  NbFilt085_min + g723shl(off_filt,2);
    if(Bound[0] > NbFilt085) Bound[0] = NbFilt085;
    Bound[1] =  NbFilt170_min + g723shl(off_filt,3);
    ...
    for ( k = 0 ; k < (int) Hb ; k ++ ) {
        ...
        for ( i = 0 ; i < (int) Bound[l] ; i ++ ) {
            ...
            Acc0 = (Word32) 0 ;
            for ( j = 0 ; j < 20 ; j ++ )
                Acc0 = g723L_add( Acc0, g723L_shr( g723L_mult(CorVct[k*20+j], *sPnt ++),
(Word16) 1 ) ) ;
    ...
    /* Modify Olp for even g723sub frames */
    if ( (Sfc & (Word16) 1 ) == (Word16) 0 ) {
    ...
    (*Line).Olp[g723shr(Sfc, (Word16) 1)] = Olp ;

    /* Decode the Acbk contribution and subtract it */
    Decod_Acbk( RezBuf, PrevExc, Olp, Lid, Gid ) ;

    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        Acc0 = g723L_deposit_h( Tv[i] ) ;
//        Acc0 = g723L_shr( Acc0, (Word16) 1 ) ;
		L_SHR(Acc0, (Word16) 1, Acc0);

        for ( j = 0 ; j <= i ; j ++ )
            Acc0 = g723L_msu( Acc0, RezBuf[j], ImpResp[i-j] ) ;
        Acc0 = g723L_shl( Acc0, (Word16) 1 ) ;
        Tv[i] = g723round( Acc0 ) ;
    }
...
Word16 Gid )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        ...
        for ( j = 0 ; j < ClPitchOrd ; j ++ )
            Acc0 = g723L_mac( Acc0, RezBuf[i+j], sPnt[j] ) ;
        Acc0 = g723L_shl( Acc0, (Word16) 1 ) ;
        Tv[i] = g723round( Acc0 ) ;
    }
...
Word16   Comp_Info( Word16 *Buff, Word16 Olp, Word16 *Gain, Word16 *ShGain)
{
    ...
    *ShGain = Vec_Norm( Buff, (Word16) (PitchMax+Frame) ) ;
    ...
    for ( i = (int)Olp-3 ; i <= (int)Olp+3 ; i ++ ) {
        ...
        Acc0 = (Word32) 0 ;
        for ( j = 0 ; j < 2*SubFrLen ; j ++ )
            Acc0 = g723L_mac( Acc0, Buff[PitchMax+Frame-2*SubFrLen+j],
    ...
    *Gain = Tenr;
    ...
    Acc0 = (Word32) 0 ;
    for ( j = 0 ; j < 2*SubFrLen ; j ++ )
        Acc0 = g723L_mac( Acc0, Buff[PitchMax+Frame-2*SubFrLen-(int)Indx+j],
    ...
    Acc0 = g723L_mult( Enr, Tenr ) ;
    Acc0 = g723L_shr( Acc0, (Word16) 3 ) ;

    Acc0 = g723L_msu( Acc0, Ccr, Ccr ) ;
...
Word16 Ecount, Word16 *Sd )
{
    ...
    if ( Ecount >= (Word16) ErrMaxNum ) {
        ...
        if ( Lag != (Word16) 0 ) {
            ...
            for ( i = 0 ; i < Frame ; i ++ )
                DataBuff[i] = Buff[PitchMax+i] = g723mult( Buff[PitchMax+i],
                            (Word16) 0x6000 ) ;
        }
        else {
            /* Unvoiced case */
            for ( i = 0 ; i < Frame ; i ++ )
                DataBuff[i] = g723mult( Gain, Rand_lbc( Sd ) ) ;
...
PFDEF Comp_Lpf( Word16 *Buff, Word16 Olp, Word16 Sfc )
{
    ...
    Acc0 = (Word32) 0 ;
    for ( j = 0 ; j < SubFrLen ; j ++ )
        Acc0 = g723L_mac( Acc0, Buff[PitchMax+(int)Sfc*SubFrLen+j],
    ...
    if ( Bindx != (Word16) 0 ) {
        ...
        for ( j = 0 ; j < SubFrLen ; j ++ ) {
            ...
            Acc1 = g723L_mac( Acc1, Buff[PitchMax+(int)Sfc*SubFrLen+(int)Bindx+j],
                        Buff[PitchMax+(int)Sfc*SubFrLen+(int)Bindx+j] ) ;
        }
        Lcr[1] = Acc0 ;
        Lcr[2] = Acc1 ;
    ...
    if ( Findx != (Word16) 0 ) {
        ...
        for ( j = 0 ; j < SubFrLen ; j ++ ) {
            ...
            Acc1 = g723L_mac( Acc1, Buff[PitchMax+(int)Sfc*SubFrLen+(int)Findx+j],
                        Buff[PitchMax+(int)Sfc*SubFrLen+(int)Findx+j] ) ;
        }
        Lcr[3] = Acc0 ;
        Lcr[4] = Acc1 ;
    ...
    if ( (Bindx != (Word16) 0) && ( Findx != (Word16) 0) ) {
        Exp = g723mult_r( Scr[1], Scr[1] ) ;
        Acc0 = g723L_mult( Exp, Scr[4] ) ;
        Exp = g723mult_r( Scr[3], Scr[3] ) ;
        Acc1 = g723L_mult( Exp, Scr[2] ) ;
        if ( Acc0 > Acc1 )
            Pf = Get_Ind( Bindx, Scr[0], Scr[1], Scr[2] ) ;
        else
            Pf = Get_Ind( Findx, Scr[0], Scr[3], Scr[4] ) ;
...
PFDEF Get_Ind( Word16 Ind, Word16 Ten, Word16 Ccr, Word16 Enr )
{
    ...
    /* Check valid gain */
    Acc0 = g723L_mult( Ten, Enr ) ;
    Acc0 = g723L_shr( Acc0, (Word16) 2 ) ;
    Acc1 = g723L_mult( Ccr, Ccr ) ;
    ...
    if ( Acc1 > Acc0 ) {
        ...
        if ( Ccr >= Enr )
            ...
            Pf.Gain = g723div_s( Ccr, Enr ) ;
            Pf.Gain = g723mult( Pf.Gain, LpfConstTable[(int)WrkRate] ) ;
        }
        /* Compute scaling gain */
        Acc0 = g723L_deposit_h( Ten ) ;
        Acc0 = g723L_shr( Acc0, (Word16) 1 ) ;
        Acc0 = g723L_mac( Acc0, Ccr, Pf.Gain ) ;
        Exp  = g723mult( Pf.Gain, Pf.Gain ) ;
        Acc1 = g723L_mult( Enr, Exp ) ;
        Acc1 = g723L_shr( Acc1, (Word16) 1 ) ;
        Acc0 = g723L_add( Acc0, Acc1 ) ;
        Exp = g723round( Acc0 ) ;

        Acc1 = g723L_deposit_h( Ten ) ;
        Acc0 = g723L_deposit_h( Exp ) ;
        Acc1 = g723L_shr( Acc1, (Word16) 1 ) ;
    ...
    Pf.Gain = g723mult( Pf.Gain, Pf.ScGn ) ;
...
//        Acc0 = L_mult( Buff[PitchMax+(int)Sfc*SubFrLen+i], Pf.ScGn ) ;
		L_MULT(Buff[PitchMax+(int)Sfc*SubFrLen+i], Pf.ScGn, Acc0);
        Acc0 = g723L_mac( Acc0, Buff[PitchMax+(int)Sfc*SubFrLen+(int)Pf.Indx+i],
                                                                Pf.Gain ) ;
        Tv[(int)Sfc*SubFrLen+i] = g723round( Acc0 ) ;
    }
 ...
 */
    ...
    if (T0 < SubFrLen-2) {
        ...
        h[i] = g723add(h[i], g723mult(h[i-T0], gain_T0));
    }
    ...
    *ind_gain = G_code(X, rr, &gain_q);
    ...
    if(T0 < SubFrLen-2)
        for (i = T0; i < SubFrLen; i++)    /* code[i] += gain_T0*code[i-T0] */
            code[i] = g723add(code[i], g723mult(code[i-T0], gain_T0));
...
void Cor_h(Word16 *H, Word16 *rr)
{
    ...
    cor = 0;
    for(i=0; i<SubFrLen; i++)
        cor = g723L_mac(cor, H[i], H[i]);

    if(g723extract_h(cor) > 32000 ) {
        for(i=0; i<SubFrLen; i++) h[i+4] = g723shr(H[i], 1);
    }
    else {
        k = g723norm_l(cor);
        k = g723shr(k, 1);
        for(i=0; i<SubFrLen; i++) h[i+4] = g723shl(H[i], k);
    }
  ...
  */
    ...
    cor    = 0;
    for(i=0;  i<NB_POS; i++) {
        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        *p3-- = g723extract_h(cor);

        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        *p2-- = g723extract_h(cor);

        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        *p1-- = g723extract_h(cor);

        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        cor = g723L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
        *p0-- = g723extract_h(cor);
    }
  ...
  */
    ...
    for(k=0; k<NB_POS; k++) {
        ...
        for(i=k+(Word16)1; i<NB_POS; i++ ) {
            ...
            cor = g723L_mac(cor, *ptr_h1, *ptr_h2); ptr_h1++; ptr_h2++;
            cor = g723L_mac(cor, *ptr_h1, *ptr_h2); ptr_h1++; ptr_h2++;
            *p3 = g723extract_h(cor);
...
void Cor_h_X(Word16 h[], Word16 X[], Word16 D[])
{
    ...
    j = g723norm_l(max);
    if( g723sub(j,16) > 0) j = 16;

    j = g723sub(18, j);

    for(i=0; i<SubFrLen; i++)
        D[i] = g723extract_l( g723L_shr(y32[i], j) );
...
*/
static Word16 extra;
_declspec(dllexport) void reset_max_time(void)
{
    extra = 120;
    return;
}
                 ...
                 Word16 y[], Word16 *code_shift, Word16 *sign)
{
  ...
  */
    ...
    max0 = g723add(max0, max1);
    max0 = g723add(max0, max2);

    /* Find means of Dn[i0]+Dn[i1]+Dn[i2] */

    L32 = 0;
    for (i = 0; i < SubFrLen; i+=STEP) {
        L32 = g723L_mac(L32, Dn[i], 1);
        L32 = g723L_mac(L32, Dn[i+2], 1);
        L32 = g723L_mac(L32, Dn[i+4], 1);
    }
    means =g723extract_l( g723L_shr(L32, 4));

    /* thres = means + (max0-means)*threshold; */

    thres = g723sub(max0, means);
    thres = g723mult(thres, threshold);
    thres = g723add(thres, means);
    ...
    max1 = g723sub(max0, means);
    max1 = g723mult(max1, threshold);
    max1 = g723add(max1, means);
  ...
  */
    ...
    for (i0 = 0; i0 < SubFrLen; i0 += STEP) {
        ...
        for (i1 = 2; i1 < SubFrLen; i1 += STEP) {
            ...
            ps1  = g723add(ps0, Dn[i1]);
            ps1a = g723add(ps0a, Dn[i1+1]);

            /* alp1 = alp0 + *ptr_ri1i1++ + 2.0 * ( *ptr_ri0i1++); */

 //           alp1 = L_mult(alp0, 1);
			L_MULT(alp0, 1, alp1);
            alp1 = g723L_mac(alp1, *ptr_ri1i1++, 1);
            alp1 = g723L_mac(alp1, *ptr_ri0i1++, 2);
            ...
            for (i2 = 4; i2 < SubFrLen2; i2 += STEP) {
                ...
                ps2  = g723add(ps1, Dn[i2]);
                ps2a = g723add(ps1a, Dn[i2+1]);

                /* alp2 = alp1 + *ptr_ri2i2++
                               + 2.0 * (*ptr_ri0i2++ + *ptr_ri1i2++); */

                alp2 = g723L_mac(alp1, *ptr_ri2i2++, 1);
                alp2 = g723L_mac(alp2, *ptr_ri0i2++, 2);
                alp2 = g723L_mac(alp2, *ptr_ri1i2++, 2);
                ...
                if ( ps2 > thres) {
                    ...
                    for (i3 = 6; i3 < SubFrLen2; i3 += STEP) {
                        ...
                        alp3 = g723L_mac(alp2, *ptr_ri3i3++, 1);
                        alp3 = g723L_mac(alp3, *ptr_ri0i3++, 2);
                        alp3 = g723L_mac(alp3, *ptr_ri1i3++, 2);
                        alp3 = g723L_mac(alp3, *ptr_ri2i3++, 2);
                        alp  = g723extract_l(g723L_shr(alp3, 5));

                        ps3c = g723mult(ps3, ps3);
                        if( g723L_mult(ps3c, alpha) > g723L_mult(psc, alp) ) {
                            psc = ps3c;
                            alpha = alp;
                            ip0 = i0;
                            ip1 = i1;
...
end_search:
    ...
    i0 = p_sign[g723shr(ip0, 1)];
    i1 = p_sign[g723shr(ip1, 1)];
    i2 = p_sign[g723shr(ip2, 1)];
    i3 = p_sign[g723shr(ip3, 1)];
    ...
    *sign = 0;
    if(i0 > 0) *sign = g723add(*sign, 1);
    if(i1 > 0) *sign = g723add(*sign, 2);
    if(i2 > 0) *sign = g723add(*sign, 4);
    if(i3 > 0) *sign = g723add(*sign, 8);

    i = g723shr(ip0, 3);
    i = g723add(i, g723shl(g723shr(ip1, 3), 3));
    i = g723add(i, g723shl(g723shr(ip2, 3), 6));
    i = g723add(i, g723shl(g723shr(ip3, 3), 9));
...
Word16 G_code(Word16 X[], Word16 Y[], Word16 *gain_q)
{
    ...
    L_xy = 0L;
    for(i=0; i<SubFrLen; i++)
        L_xy = g723L_mac(L_xy, X[i], Y[i]);

    exp_xy = g723norm_l(L_xy);
    xy = g723extract_h( g723L_shl(L_xy, exp_xy) );
    ...
    L_yy = 0L;
    for(i=0; i<SubFrLen; i++)
        L_yy = g723L_mac(L_yy, Y[i], Y[i]);

    exp_yy = g723norm_l(L_yy);
    yy     = g723extract_h( g723L_shl(L_yy, exp_yy) );

    /* compute gain = xy/yy */
    xy = g723shr(xy, 1);             /* Be sure xy < yy */
    gain_nq = g723div_s( xy, yy);

    i = g723add(exp_xy, 5);          /* Denormalization of division */
    i = g723sub(i, exp_yy);

    gain_nq = g723shr(gain_nq, i);

    gain = (Word16) 0;
    dist_min = g723sub(gain_nq, FcbkGainTable[0]);
    dist_min = g723abs_s(dist_min);
    for ( i =  1; i <NumOfGainLev ; i ++ ) {
        dist = g723sub(gain_nq, FcbkGainTable[i]);
        dist =g723abs_s(dist);
        if ( dist< dist_min) {
            dist_min = dist;
            gain = (Word16) i ;
