...
void AtoLsp( Word16 *LspVect, Word16 *Lpc, Word16 *PrevLsp )
{
  ...
  */
    for ( i = 0 ; i < LpcOrder ; i ++ )
        LspVect[i] = g723mult_r( Lpc[i], BandExpTable[i] ) ;
  ...
  */
    for ( i = 0 ; i < LpcOrder/2 ; i ++ ) {
        ...
        /* P(z) */
        Acc0 = g723L_negate( Lpq[2*i+0] ) ;
        Acc1 = g723L_deposit_h( LspVect[i] ) ;
        Acc1 = g723L_shr( Acc1, (Word16) 4 ) ;
        Acc0 = g723L_sub( Acc0, Acc1 ) ;
        Acc1 = g723L_deposit_h( LspVect[LpcOrder-1-i] ) ;
        Acc1 = g723L_shr( Acc1, (Word16) 4 ) ;
        Acc0 = g723L_sub( Acc0, Acc1 ) ;
        Lpq[2*i+2] = Acc0 ;
  ...
  * evaluation.
  */
    Lpq[LpcOrder+0] = g723L_shr( Lpq[LpcOrder+0], (Word16) 1 ) ;
    Lpq[LpcOrder+1] = g723L_shr( Lpq[LpcOrder+1], (Word16) 1 ) ;
  ...
  */
    ...
    /* Find the maximum */
    Acc1 = g723L_abs( Lpq[0] ) ;
    for ( i = 1 ; i < LpcOrder+2 ; i ++ ) {
        Acc0 = g723L_abs( Lpq[i] ) ;
        if ( Acc0 > Acc1 )
            Acc1 = Acc0 ;
    }
    ...
    for ( i = 0 ; i < LpcOrder+2 ; i ++ ) {
        Acc0 = g723L_shl( Lpq[i], Exp ) ;
        Spq[i] = g723round( Acc0 ) ;
    }
  ...
  */
    ...
    for ( j = 0 ; j <= LpcOrder/2 ; j ++ )
        PrevVal = g723L_mac( PrevVal, Spq[2*j], CosineTable[0] ) ;
  ...
  */
    ...
    for ( i = 1 ; i < CosineTableSize/2 ; i ++ ) {
        ...
        CurrVal = (Word32) 0 ;
        for ( j = 0 ; j <= LpcOrder/2 ; j ++ )
            CurrVal = g723L_mac( CurrVal, Spq[LpcOrder-2*j+k],
  ...
  */
            Acc0 = g723L_abs( CurrVal ) ;
            Acc1 = g723L_abs( PrevVal ) ;
 //           Acc0 = g723L_add( Acc0, Acc1 ) ;
			L_ADD(Acc0, Acc1, Acc0);
            ...
            Acc1 = g723L_shl( Acc1, Exp ) ;

            Acc1 = g723L_shr( Acc1, (Word16) 8 ) ;

            LspVect[LspCnt] = div_l( Acc1, g723extract_h( Acc0 ) ) ;

 /*
  * Add the upper part of the zero-crossing frequency,
  * i.e. bits 7-15
  */
            Exp = g723shl( (Word16) (i-1), (Word16) 7 ) ;
            LspVect[LspCnt] = g723add( LspVect[LspCnt], Exp ) ;
            LspCnt ++ ;
...
**          vector is divided into 3 g723sub-vectors, or
...
Word32   Lsp_Qnt( Word16 *CurrLsp, Word16 *PrevLsp )
{
  ...
  */
    ...
    Wvect[0] = g723sub( CurrLsp[1], CurrLsp[0] ) ;
    Wvect[LpcOrder-1] = g723sub( CurrLsp[LpcOrder-1], CurrLsp[LpcOrder-2] ) ;

    /* Compute the rest of the differences */
    for ( i = 1 ; i < LpcOrder-1 ; i ++ ) {
        Tmp0 = g723sub( CurrLsp[i+1], CurrLsp[i] ) ;
        Tmp1 = g723sub( CurrLsp[i], CurrLsp[i-1] ) ;
        if ( Tmp0 > Tmp1 )
            Wvect[i] = Tmp1 ;
        else
            Wvect[i] = Tmp0 ;
    ...
    for ( i = 0 ; i < LpcOrder ; i ++ ) {
        ...
        if ( Wvect[i] > Tmp0 )
            Wvect[i] = g723div_s( Tmp0, Wvect[i] ) ;
        else
            Wvect[i] = MAX_16 ;
    }
  ...
  */
    for ( i = 0 ; i < LpcOrder ; i ++ ) {
        CurrLsp[i] = g723sub( CurrLsp[i], LspDcTable[i] ) ;
        PrevLsp[i] = g723sub( PrevLsp[i], LspDcTable[i] ) ;
    }
  ...
  */
    for ( i = 0 ; i < LpcOrder ; i ++ ) {
        Tmp0 = g723mult_r( PrevLsp[i], (Word16) LspPrd0 ) ;
        CurrLsp[i] = g723sub( CurrLsp[i], Tmp0 ) ;
    }
  ...
  */
    for ( i = 0 ; i < LpcOrder ; i ++ )
        PrevLsp[i] = g723add( PrevLsp[i], LspDcTable[i] ) ;
...
Word32   Lsp_Svq( Word16 *Tv, Word16 *Wvect )
{
  ...
  */
            for ( j = 0 ; j < BandInfoTable[k][1] ; j ++ )
                Tmp[j] = g723mult_r( Wvect[BandInfoTable[k][0]+j],
                                                            LspQntPnt[j] ) ;

            Acc0 = (Word32) 0 ;
            for ( j = 0 ; j < BandInfoTable[k][1] ; j ++ )
                Acc0 = g723L_mac( Acc0, Tv[BandInfoTable[k][0]+j], Tmp[j] ) ;
            Acc0 = g723L_shl( Acc0, (Word16) 1 ) ;
            for ( j = 0 ; j < BandInfoTable[k][1] ; j ++ )
                Acc0 = g723L_msu( Acc0, LspQntPnt[j], Tmp[j] ) ;
  ...
  */
        Rez = g723L_shl( Rez, (Word16) LspCbBits ) ;
 //       Rez = g723L_add( Rez, Indx ) ;
		L_ADD(Rez, Indx, Rez);
    }
...
**          into 3 g723sub-vectors, or bands, of dimension 3,
...
void Lsp_Inq( Word16 *Lsp, Word16 *PrevLsp, Word32 LspId, Word16 Crc )
{
  ...
  */
    for ( j = 0 ; j < LpcOrder ; j ++ )
        PrevLsp[j] = g723sub(PrevLsp[j], LspDcTable[j] ) ;
  ...
  */
    for ( j = 0 ; j < LpcOrder ; j ++ ) {
        Tmp = g723mult_r( PrevLsp[j], Lprd ) ;
        Lsp[j] = g723add( Lsp[j], Tmp ) ;
    }
  ...
  */
    for ( j = 0 ; j < LpcOrder ; j ++ ) {
        PrevLsp[j] = g723add( PrevLsp[j], LspDcTable[j] ) ;
        Lsp[j] = g723add( Lsp[j], LspDcTable[j] ) ;
    }
  ...
  */
    for ( i = 0 ; i < LpcOrder ; i ++ ) {
        ...
        for ( j = 1 ; j < LpcOrder ; j ++ ) {
            ...
            Tmp = g723add( Scon, Lsp[j-1] ) ;
            Tmp = g723sub( Tmp, Lsp[j] ) ;
            if ( Tmp > (Word16) 0 ) {
                Tmp = g723shr( Tmp, (Word16) 1 ) ;
                Lsp[j-1] = g723sub( Lsp[j-1], Tmp ) ;
                Lsp[j] = g723add( Lsp[j], Tmp ) ;
            }
        }
  ...
  */
        for ( j = 1 ; j < LpcOrder ; j ++ ) {
            Tmp = g723add( Lsp[j-1], Scon ) ;
            Tmp = g723sub( Tmp, (Word16) 4 ) ;
            Tmp = g723sub( Tmp, Lsp[j] ) ;
            if ( Tmp > (Word16) 0 )
                Test = True ;
        }
...
void  Lsp_Int( Word16 *QntLpc, Word16 *CurrLsp, Word16 *PrevLsp )
{
  ...
  */
        for ( j = 0 ; j < LpcOrder ; j ++ ) {
            Acc0 = g723L_deposit_h( PrevLsp[j] ) ;
            Acc0 = g723L_mac( Acc0, Tmp, PrevLsp[j] ) ;
            Acc0 = g723L_msu( Acc0, Tmp, CurrLsp[j] ) ;
            Dpnt[j] = g723round( Acc0 ) ;
        }
  ...
  */
        ...
        Tmp = g723add( Tmp, (Word16) (MIN_16 / SubFrames ) ) ;
    }
...
void  LsptoA( Word16 *Lsp )
{
  ...
  */
        j = (int) g723shr( Lsp[i], (Word16) 7 ) ;
        Acc0 = g723L_deposit_h( CosineTable[j] ) ;

 /*
  * Do the linear interpolations using bits [6:0] of the LSP
  * frequency
  */
        Tmp = g723sub(CosineTable[j+1], CosineTable[j] ) ;
        Acc0 = g723L_mac( Acc0, Tmp, g723add( g723shl( (Word16)(Lsp[i] & 0x007f) ,
                                (Word16)8 ), (Word16) 0x0080 ) ) ;
        Acc0 = g723L_shl( Acc0, (Word16) 1 ) ;
        Lsp[i] = g723negate( g723round( Acc0 ) ) ;
    }
  ...
  */
    ...
    P[1] = g723L_mult( Lsp[0], (Word16) 0x2000 ) ;
    P[1] = g723L_mac( P[1], Lsp[2], (Word16) 0x2000 ) ;
    P[2] = g723L_mult( Lsp[0], Lsp[2] ) ;
    P[2] = g723L_shr( P[2], (Word16) 1 ) ;
    P[2] = g723L_add( P[2], (Word32) 0x20000000L ) ;

    Q[0] = (Word32) 0x10000000L ;
    Q[1] = g723L_mult( Lsp[1], (Word16) 0x2000 ) ;
    Q[1] = g723L_mac( Q[1], Lsp[3], (Word16) 0x2000 ) ;
    Q[2] = g723L_mult( Lsp[1], Lsp[3] ) ;
    Q[2] = g723L_shr( Q[2], (Word16) 1 ) ;
    Q[2] = g723L_add( Q[2], (Word32) 0x20000000L ) ;
 ...
 //       Acc0 = g723L_add( Acc0, P[i-1] ) ;
		L_ADD(Acc0, P[i-1], Acc0);
        P[i+1] = Acc0 ;

        Acc1 = Q[i] ;
        Acc1 = L_mls( Acc1, Lsp[2*i+1] ) ;
//        Acc1 = g723L_add( Acc1, Q[i-1] ) ;
        ...
        for ( j = i ; j >= 2 ; j -- ) {
            Acc0 = P[j-1] ;
            Acc0 = L_mls( Acc0, Lsp[2*i+0] ) ;
            Acc0 = g723L_add( Acc0, g723L_shr(P[j], (Word16) 1 ) ) ;
            Acc0 = g723L_add( Acc0, g723L_shr(P[j-2], (Word16) 1 ) ) ;
            P[j] = Acc0 ;

            Acc1 = Q[j-1] ;
            Acc1 = L_mls( Acc1, Lsp[2*i+1] ) ;
            Acc1 = g723L_add( Acc1, g723L_shr(Q[j], (Word16) 1 ) ) ;
            Acc1 = g723L_add( Acc1, g723L_shr(Q[j-2], (Word16) 1 ) ) ;
            Q[j] = Acc1 ;
        }

        /* Compute coefficients 1, 0 */
        P[0] = g723L_shr( P[0], (Word16) 1 ) ;
        Q[0] = g723L_shr( Q[0], (Word16) 1 ) ;

        Acc0 = g723L_deposit_h( Lsp[2*i+0] ) ;
        Acc0 = g723L_shr( Acc0, (Word16) i ) ;
        Acc0 = g723L_add( Acc0, P[1] ) ;
        Acc0 = g723L_shr( Acc0, (Word16) 1 ) ;
        P[1] = Acc0 ;

        Acc1 = g723L_deposit_h( Lsp[2*i+1] ) ;
        Acc1 = g723L_shr( Acc1, (Word16) i ) ;
        Acc1 = g723L_add( Acc1, Q[1] ) ;
  ...
  */
    for ( i = 0 ; i < LpcOrder/2 ; i ++ ) {
        Acc0 = P[i] ;
        Acc0 = g723L_add( Acc0, P[i+1] ) ;
        Acc0 = g723L_sub( Acc0, Q[i] ) ;
        Acc0 = g723L_add( Acc0, Q[i+1] ) ;
        Acc0 = g723L_shl( Acc0, (Word16) 3 ) ;
        Lsp[i] = g723negate( g723round( Acc0 ) ) ;

        Acc1 = P[i] ;
        Acc1 = g723L_add( Acc1, P[i+1] ) ;
        Acc1 = g723L_add( Acc1, Q[i] ) ;
        Acc1 = g723L_sub( Acc1, Q[i+1] ) ;
        Acc1 = g723L_shl( Acc1, (Word16) 3 ) ;
        Lsp[LpcOrder-1-i] = g723negate( g723round( Acc1 ) ) ;
    }
