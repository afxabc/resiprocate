...
//            Acc0 = L_mult( Dpnt[i], (Word16) 0x4000 ) ;
			L_MULT(Dpnt[i], (Word16) 0x4000, Acc0);
            Acc0 = g723L_mac ( Acc0, CodStat.HpfZdl, (Word16) 0xc000 ) ;
        ...
        for ( i = 0 ; i < Frame ; i ++ )
            Dpnt[i] = g723shr( Dpnt[i], (Word16) 1 ) ;
    }
...
Word16  Vec_Norm( Word16 *Vect, Word16 Len )
{
 ...
 //       Temp = L_mult( Exp, Vect[i] ) ;
		L_MULT(Exp, Vect[i], Temp);
        Temp = g723L_shr( Temp, 4 ) ;
        Vect[i] = g723extract_l( Temp ) ;
    }

    Rez = g723sub( Rez, (Word16) 3) ;
    return Rez ;
}
...
Word32   Comp_En( Word16 *Dpnt )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ )
        Temp[i] = g723shr( Dpnt[i], (Word16) 2 ) ;

    Rez = (Word32) 0 ;
    for ( i = 0 ; i < SubFrLen ; i ++ )
        Rez = g723L_mac( Rez, Temp[i], Temp[i] ) ;
...
//        Acc = L_mult( g723add(Rez, Exp), g723add(Rez, Exp) ) ;
		L_MULT(g723add(Rez, Exp), g723add(Rez, Exp), Acc);
        if ( Num >= Acc )
            Rez = g723add( Rez, Exp ) ;

        Exp = g723shr( Exp, (Word16) 1 ) ;
    }
    return Rez ;
}
...
Word16   Rand_lbc( Word16 *p )
{
    ...
    Temp = g723L_deposit_l( *p ) ;
    Temp &= (Word32) 0x0000ffff ;
    Temp = Temp*(Word32)521 + (Word32) 259 ;
    *p = g723extract_l( Temp ) ;
    return g723extract_l( Temp ) ;
}
...
void  Scale( Word16 *Tv, Word32 Sen )
{
    ...
    if ( (Acc1 != (Word32) 0) && (Acc0 != (Word32) 0 ) ) {
        ...
        SfGain = g723norm_l( Acc0 ) ;
        Acc0 = g723L_shl( Acc0, SfGain ) ;
        Acc0 = g723L_shr( Acc0, (Word16) 1 ) ;
        Exp = g723sub( Exp, SfGain ) ;
        Exp = g723add( Exp, (Word16) 1 ) ;
        Exp = g723sub( (Word16) 6, Exp ) ;
        if ( Exp < (Word16) 0 )
            Exp = (Word16) 0 ;

        SfGain = g723extract_h( Acc1 ) ;

        SfGain = div_l( Acc0, SfGain ) ;

        Acc0 = g723L_deposit_h( SfGain ) ;

        Acc0 = g723L_shr( Acc0, Exp ) ;
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        ...
        /* Update gain */
        Acc0 = g723L_deposit_h( DecStat.Gain ) ;
        Acc0 = g723L_msu( Acc0, DecStat.Gain, (Word16) 0x0800 ) ;
        Acc0 = g723L_mac( Acc0, SfGain, (Word16) 0x0800 ) ;
        DecStat.Gain = g723round( Acc0 ) ;

        Exp = g723add( DecStat.Gain, g723shr( DecStat.Gain, (Word16) 4) ) ;

//        Acc0 = L_mult( Tv[i], Exp ) ;
		L_MULT(Tv[i], Exp, Acc0);
        Acc0 = g723L_shl( Acc0, (Word16) 4 ) ;
        Tv[i] = g723round( Acc0 ) ;
    }
