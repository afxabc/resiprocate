  ...
  */
    ...
    for(iblk=0; iblk<SubFrames/2; iblk++) {
        ...
        *ptr1++ = temp & (Word16)0x0001;
        temp    = g723shr(temp, 1);
        *ptr1++ = g723add( (Word16) SubFrLen, (Word16) (temp & 0x0001) );
        for(i=0; i<NbPulsBlk; i++) {
            *ptr_TabSign++= g723shl(g723sub((temp & (Word16)0x0002), 1), 14);
            temp = g723shr(temp, 1);
        }
    }
    ...
    for(i_subfr=0; i_subfr<SubFrames; i_subfr++) {
        ...
        for(i=0; i<Nb_puls[i_subfr]; i++) {
            ...
            *ptr_TabPos++ = g723add(g723shl(tmp[(int)j],1), offset[i_subfr]);
            temp = g723sub(temp, 1);
  ...
  */
    ...
    for(iblk=0; iblk<SubFrames/2; iblk++) {
        ...
        for(i=0; i<SubFrLenD; i++) {
            temp = g723abs_s(curExc[i]);
            if(temp > temp2) temp2 = temp;
        }
        if(temp2 == 0) sh1 = 0;
        else {
            sh1 = g723sub(4,g723norm_s(temp2)); /* 4 bits of margin  */
            if(sh1 < -2) sh1 = -2;
        }

        L_temp = 0L;
        for(i=0; i<SubFrLenD; i++) {
            temp  = g723shr(curExc[i], sh1); /* left if sh1 < 0 */
            L_temp = g723L_mac(L_temp, temp, temp);
            tempExc[i] = temp;
        }  /* ener_ltp x 2**(-2sh1+1) */

        L_acc = 0L;
        for(i=0; i<NbPulsBlk; i++) {
            L_acc = g723L_mac(L_acc, tempExc[(int)ptr_TabPos[i]], ptr_TabSign[i]);
        }
        inter_exc = g723extract_h(g723L_shl(L_acc, 1)); /* inter_exc x 2-sh1 */
...
//        L_acc = L_mult(curGain, SubFrLen);
        ...
        L_acc = g723L_shr(L_acc, 6);
        temp  = g723extract_l(L_acc);   /* SubFrLen x curGain : avoids overflows */
 //       L_acc = L_mult(temp, curGain);
		L_MULT(temp, curGain, L_acc);
        temp = g723shl(sh1, 1);
        temp = g723add(temp, 4);
        L_acc = g723L_shr(L_acc, temp); /* SubFrLenD x curGain**2 x 2**(-2sh1+1) */

        /* c = (ener_ltp - SubFrLenD x curGain**2)/nb_pulses_blk */
        /* compute L_c = c >> 2sh1-1                                */
        L_acc = g723L_sub(L_temp, L_acc);
        /* x 1/nb_pulses_blk */
 ...
 */
        /* delta = b0 x b0 - c */
        b0 = g723mult_r(inter_exc, InvNbPulsBlk);   /* b0 >> sh1 */
        L_acc = g723L_msu(L_c, b0, b0);             /* (c - b0**2) >> 2sh1-1 */
        L_acc = g723L_negate(L_acc);                /* delta x 2**(-2sh1+1) */

        /* Case delta <= 0 */
        if(L_acc <= 0) {  /* delta <= 0 */
            x1 = g723negate(b0);        /* sh1 */
        }

        /* Case delta > 0 */
        else {
            delta = Sqrt_lbc(L_acc);  /* >> sh1 */
            x1 = g723sub(delta, b0);      /* x1 >> sh1 */
            x2 = g723add(b0, delta);      /* (-x2) >> sh1 */
            if(g723abs_s(x2) < g723abs_s(x1)) {
                x1 = g723negate(x2);
            }
        }
        ...
        for(i=0; i<NbPulsBlk; i++) {
            j = *ptr_TabPos++;
            curExc[(int)j] = g723add(curExc[(int)j], g723mult(temp,
                                                (*ptr_TabSign++)) );
        }
...
Word16 Qua_SidGain(Word16 *Ener, Word16 *shEner, Word16 nq)
{
    ...
    if(nq == 0) {
         ...
         temp = g723shl(*shEner, 1);
         temp = g723sub(16, temp);
         L_acc = g723L_deposit_l(*Ener);
         L_acc = g723L_shl(L_acc, temp); /* may overflow, and >> if temp < 0 */
  ...
  */
         ...
         for(i=0, L_x=0L; i<nq; i++) {
             temp = g723sub(shEner[i], sh1);
             temp = g723shr(Ener[i], temp);
             temp = g723mult_r(fact[nq], temp);
             L_x = g723L_add(L_x, g723L_deposit_l(temp));
         }
         temp = g723sub(15, sh1);
         L_x = g723L_shl(L_x, temp);
    }
    ...
    iseg_p1 = g723add(iseg,1);
    j = g723shl(1, exp);
    k = g723shr(j,1);

    /* Binary search in segment iseg */
    for(i=0; i<exp; i++) {
        temp = g723add(base[iseg], g723shl(j, iseg_p1));
//        L_y = L_mult(temp, temp);
		L_MULT(temp, temp, L_y);
        if(L_x >= L_y) j = g723add(j, k);
        else j = g723sub(j, k);
        k = g723shr(k, 1);
    }

    temp = g723add(base[iseg], g723shl(j, iseg_p1));
    L_y = g723L_mult(temp, temp);
    L_y = g723L_sub(L_y, L_x);
    if(L_y <= 0L) {
        j2    = g723add(j, 1);
        temp  = g723add(base[iseg], g723shl(j2, iseg_p1));
        L_acc = g723L_mult(temp, temp);
        L_acc = g723L_sub(L_x, L_acc);
        if(L_y > L_acc) temp = g723add(g723shl(iseg,4), j);
        else temp = g723add(g723shl(iseg,4), j2);
    }
    else {
        j2    = g723sub(j, 1);
        temp  = g723add(base[iseg], g723shl(j2, iseg_p1));
...
Word16 Dec_SidGain(Word16 iGain)
{
    ...
    iseg = g723shr(iGain, 4);
    if(iseg == 3) iseg = 2;
    i = g723sub(iGain, g723shl(iseg, 4));
    temp = g723add(iseg, 1);
    temp = g723shl(i, temp);
    temp = g723add(temp, base[iseg]);  /* SidGain */
    temp = g723shl(temp, 5); /* << 5 */
    return(temp);
}
