...
**
*/
_declspec(dllexport) void  Init_Decod( )
{
    int   i  ;
...
_declspec(dllexport) Flag Decod( Word16 *DataBuff, char *Vinp, Word16 Crc )
{
    int   i,j   ;
    ...
    */
    ...
    if(Ftyp != 1) {
        ...
        */
        if ( Line.Crc != (Word16) 0 )
            DecStat.Ecount = g723add( DecStat.Ecount, (Word16) 1 ) ;
        else
        ...
        */
        if ( DecStat.Ecount == (Word16) 0 ) {
            DecStat.InterGain = g723add( Line.Sfs[SubFrames-2].Mamp,
                                            Line.Sfs[SubFrames-1].Mamp ) ;
            DecStat.InterGain = g723shr( DecStat.InterGain, (Word16) 1 ) ;
            DecStat.InterGain = FcbkGainTable[DecStat.InterGain] ;
        }
        else
            DecStat.InterGain = g723mult_r( DecStat.InterGain, (Word16) 0x6000 ) ;
        ...
        if ( DecStat.Ecount == (Word16) 0 ) {
            ...
            for ( i = 0 ; i < SubFrames ; i ++ ) {
                ...
                for ( j = 0 ; j < SubFrLen ; j ++ ) {
                    Dpnt[j] = g723shl( Dpnt[j], (Word16) 1 ) ;
                    Dpnt[j] = g723add( Dpnt[j], AcbkCont[j] ) ;
                }
