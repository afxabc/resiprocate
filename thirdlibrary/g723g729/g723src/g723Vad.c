...
_declspec(dllexport) void Init_Vad(void)
{
    int i ;
    VadStat.Hcnt = 3 ;
    VadStat.Vcnt = 0 ;
...
Flag Comp_Vad( Word16 *Dpnt)
{
    ...
    for ( i = 0 ; i < 4 ; i ++ ) {
        Tm1 = Minp ;
        for ( j = 0 ; j < 8 ; j ++ ) {
            Tm0 = g723sub( Tm1, VadStat.Polp[i] ) ;
            Tm0 = g723abs_s( Tm0 ) ;
            if ( Tm0 <= 3 )
                Tm2 ++ ;
            Tm1 = g723add( Tm1, Minp ) ;
        }
    }
...
//        Acc0 = L_mult( Dpnt[i], 0x2000 ) ;
		L_MULT(Dpnt[i], 0x2000, Acc0);
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_msu( Acc0, Dpnt[i-j-1], VadStat.NLpc[j] ) ;
        Tm0 = g723round ( Acc0 ) ;
        Acc1 = g723L_mac( Acc1, Tm0, Tm0 ) ;
    }
    ...
    if ( VadStat.Nlev > VadStat.Penr ) {
        Acc0 = g723L_sub( VadStat.Penr, g723L_shr( VadStat.Penr, 2 ) ) ;
        VadStat.Nlev = g723L_add( Acc0, g723L_shr( VadStat.Nlev, 2 ) ) ;
    }


    /* Update the noise level, if adaptation is enabled */
    if ( !VadStat.Aen ) {
        VadStat.Nlev = g723L_add( VadStat.Nlev, g723L_shr( VadStat.Nlev, 5 ) ) ;
    }
    /* Decay Nlev by small amount */
    else {
        VadStat.Nlev = g723L_sub( VadStat.Nlev, g723L_shr( VadStat.Nlev,11 ) ) ;
    }
    ...
    Acc0 = g723L_shl( VadStat.Nlev, 13 ) ;
    Tm0 = g723norm_l( Acc0 ) ;
    Acc0 = g723L_shl( Acc0, Tm0 ) ;
    Acc0 &= 0x3f000000L ;
    Acc0 <<= 1 ;
    Tm1 = g723extract_h( Acc0 ) ;
    Acc0 = g723L_deposit_h( ScfTab[Tm0] ) ;
    Acc0 = g723L_mac( Acc0, Tm1, ScfTab[Tm0-1] ) ;
    Acc0 = g723L_msu( Acc0, Tm1, ScfTab[Tm0] ) ;
    Tm1 = g723extract_h( Acc0 ) ;
    Tm0 = g723extract_l( g723L_shr( VadStat.Nlev, 2 ) ) ;
    Acc0 = g723L_mult( Tm0, Tm1 ) ;
    Acc0 >>= 11 ;
