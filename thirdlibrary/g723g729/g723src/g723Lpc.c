...
void  Comp_Lpc( Word16 *UnqLpc, Word16 *PrevDat, Word16 *DataBuff )
{
    ...
    for ( k = 0 ; k < SubFrames ; k ++ ) {
        ...
        */
        ...
        for ( i = 0 ; i < LpcFrame ; i ++ )
            Vect[i] = g723mult_r(Vect[i], HammingWindowTable[i]) ;
...
//            Acc0 = L_mult( Vect[i], Vect[i] ) ;
			L_MULT(Vect[i], Vect[i], Acc0);
            Acc0 = g723L_shr( Acc0, (Word16) 1 ) ;
//            Acc1 = g723L_add( Acc1, Acc0 ) ;
			L_ADD(Acc1, Acc0, Acc1);
        }

        /* Apply a white noise correction factor of (1025/1024) */
        Acc0 = g723L_shr( Acc1, (Word16) RidgeFact ) ;
        Acc1 = g723L_add( Acc1, Acc0 ) ;
 ...
 //                   Acc0 = L_mult( Vect[j], Vect[j-i] ) ;
					L_MULT(Vect[j], Vect[j-i], Acc0);
                    Acc0 = g723L_shr( Acc0, (Word16) 1 ) ;
 //                   Acc1 = g723L_add( Acc1, Acc0 ) ;
					L_ADD(Acc1, Acc0, Acc1);
                }
                Acc0 = g723L_shl( Acc1, Exp ) ;
                Acc0 = L_mls( Acc0, BinomialWindowTable[i-1] ) ;
                curAcf[i] = g723round(Acc0) ;
            }
            /* Save Acf scaling factor */
            ShAcf_sf[k] = g723add(Exp, g723shl(ShAcf_sf[k], 1));
        }
 ...
 */
        ...
        Acc0 = g723L_deposit_h( Corr[i] ) ;
        Acc0 = g723L_shr( Acc0, (Word16) 2 ) ;
        for ( j = 0 ; j < i ; j ++ )
            Acc0 = g723L_msu( Acc0, Lpc[j], Corr[i-j-1] ) ;
  ...
  */
        Acc0 = g723L_deposit_h( g723negate(Pk) ) ;
        Acc0 = g723L_shr( Acc0, (Word16) 2 ) ;
        Lpc[i] = g723round( Acc0 ) ;

 /*
  * Update the prediction error
  */
        Acc1 = L_mls( Acc1, Pk ) ;
 //       Acc1 = g723L_add( Acc1, Acc2 ) ;
		L_ADD(Acc1, Acc2, Acc1);
        Err = g723round( Acc1 ) ;
  ...
  */
        ...
        for ( j = 0 ; j < i ; j ++ ) {
            Acc0 = g723L_deposit_h( Lpc[j] ) ;
            Acc0 = g723L_mac( Acc0, Pk, Temp[i-j-1] ) ;
            Lpc[j] = g723round( Acc0 ) ;
        }
    }
...
void  Wght_Lpc( Word16 *PerLpc, Word16 *UnqLpc )
{
  ...
  */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            PerLpc[j] = g723mult_r( UnqLpc[j], PerFiltZeroTable[j] ) ;
        PerLpc += LpcOrder ;
 ...
 */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            PerLpc[j] = g723mult_r( UnqLpc[j], PerFiltPoleTable[j] ) ;
        PerLpc += LpcOrder ;
        UnqLpc += LpcOrder ;
    }
}
...
//            Acc0 = L_mult( *Dpnt, (Word16) 0x2000 ) ;
			L_MULT(*Dpnt, (Word16) 0x2000, Acc0);
            for ( j = 0 ; j < LpcOrder ; j ++ )
                Acc0 = g723L_msu( Acc0, PerLpc[j], CodStat.WghtFirDl[j] ) ;
  ...
  */
            ...
            /* Filter */
            for ( j = 0 ; j < LpcOrder ; j ++ )
                Acc0 = g723L_mac( Acc0, PerLpc[LpcOrder+j],
                                                    CodStat.WghtIirDl[j] ) ;
            for ( j = LpcOrder-1 ; j > 0 ; j -- )
            ...
            /* Update memory */
            CodStat.WghtIirDl[0] = g723round( Acc0 ) ;
...
void  Comp_Ir( Word16 *ImpResp, Word16 *QntLpc, Word16 *PerLpc, PWDEF Pw )
{
  ...
  */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_mac( Acc0, QntLpc[j], FirDl[j] ) ;
        Acc1 = g723L_shl( Acc0, (Word16) 2 ) ;
  ...
  */
        ...
        /* FIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_msu( Acc0, PerLpc[j], FirDl[j] ) ;
                                Acc0 = g723L_shl( Acc0, (Word16) 1 ) ;
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            FirDl[j] = FirDl[j-1] ;
        FirDl[0] = g723round( Acc1 ) ;

        /* Iir part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_mac( Acc0, PerLpc[LpcOrder+j], IirDl[j] ) ;
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            IirDl[j] = IirDl[j-1] ;
        Acc0 = g723L_shl( Acc0, (Word16) 2 ) ;
        IirDl[0] = g723round( Acc0 ) ;
  ...
  */
        ...
        Acc0 = g723L_deposit_h( IirDl[0] ) ;
        Acc0 = g723L_msu( Acc0, Pw.Gain, Temp[PitchMax-Pw.Indx+i] ) ;
        ImpResp[i] = g723round( Acc0 ) ;
...
*PrevErr, PWDEF Pw )
{
  ...
  */
        Acc0 = g723L_deposit_h( g723sub( Dpnt[i], IirDl[0] ) ) ;
        Acc0 = g723L_mac( Acc0, Pw.Gain, Temp[PitchMax-(int)Pw.Indx+i] ) ;
        Dpnt[i] = g723round ( Acc0 ) ;
    }
}
...
*PrevErr )
{
  ...
  */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_mac( Acc0, QntLpc[j], CodStat.RingFirDl[j] ) ;
        Acc1 = g723L_shl( Acc0, (Word16) 2 ) ;

        Dpnt[i] = g723shl( g723round( Acc1 ), (Word16) 1 ) ;
  ...
  */
        ...
        /* FIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_msu( Acc0, PerLpc[j], CodStat.RingFirDl[j] ) ;

        /* Update FIR memory */
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            CodStat.RingFirDl[j] = CodStat.RingFirDl[j-1] ;
        CodStat.RingFirDl[0] = g723round( Acc1 ) ;

        /* IIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_mac( Acc0, PerLpc[LpcOrder+j], CodStat.RingIirDl[j] ) ;
        Acc0 = g723L_shl( Acc0, (Word16) 2 ) ;

        /* Update IIR memory */
        for ( j = LpcOrder-1 ; j > 0 ; j -- )
            CodStat.RingIirDl[j] = CodStat.RingIirDl[j-1] ;
        CodStat.RingIirDl[0] = g723round( Acc0 ) ;
...
void     Synt( Word16 *Dpnt, Word16 *Lpc )
{
  ...
  */
        ...
        /* Filter */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_mac( Acc0, Lpc[j], DecStat.SyntIirDl[j] ) ;
        ...
        DecStat.SyntIirDl[0] = g723round( Acc0 ) ;
  ...
  */
        if ( UsePf )
            ...
            Dpnt[i] = g723shl( DecStat.SyntIirDl[0], (Word16) 1 ) ;
...
Word32  Spf( Word16 *Tv, Word16 *Lpc )
{
  ...
  */
    for ( i = 0 ; i < LpcOrder ; i ++ ) {
        FirCoef[i] = g723mult_r( Lpc[i], PostFiltZeroTable[i] ) ;
        IirCoef[i] = g723mult_r( Lpc[i], PostFiltPoleTable[i] ) ;
    }
  ...
  */
    Acc0 = (Word32) 0 ;
    Acc1 = g723L_mult( TmpVect[0], TmpVect[0] ) ;
    for ( i = 1 ; i < SubFrLen ; i ++ ) {
        Acc0 = g723L_mac( Acc0, TmpVect[i], TmpVect[i-1] ) ;
        Acc1 = g723L_mac( Acc1, TmpVect[i], TmpVect[i] ) ;
    }

 /*
  * Scale the energy for the later use.
  */
    Sen = g723L_shr( Acc1, (Word16)(2*Exp + 4) ) ;

 /*
  * Compute the first-order partial correlation coefficient of the
  * input speech vector.
  */
    Tmp = g723extract_h( Acc1 ) ;
    if ( Tmp != (Word16) 0 ) {
  ...
  */
    Acc0 = g723L_deposit_h( DecStat.Park ) ;
    Acc0 = g723L_msu( Acc0, DecStat.Park, (Word16) 0x2000 ) ;
    Acc0 = g723L_mac( Acc0, Tmp, (Word16) 0x2000 ) ;
    DecStat.Park = g723round( Acc0 ) ;

    Tmp  = g723mult( DecStat.Park, PreCoef ) ;
    Tmp &= (Word16) 0xfffc ;
  ...
  */
        ...
        /* FIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_msu( Acc0, FirCoef[j], DecStat.PostFirDl[j] ) ;
        ...
        /* IIR part */
        for ( j = 0 ; j < LpcOrder ; j ++ )
            Acc0 = g723L_mac( Acc0, IirCoef[j], DecStat.PostIirDl[j] ) ;
        ...
        DecStat.PostIirDl[0] = g723round( Acc0 ) ;

 /*
  * Compensation filter
  */
        Acc1 = g723L_mac( Acc1, DecStat.PostIirDl[1], Tmp ) ;
