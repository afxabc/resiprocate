...
**
*/
_declspec(dllexport) void Init_Cod_Cng(void)
{
    int i;
  ...
  */
        if(LpcDiff(CodCng.RC, CodCng.ShRC, CodCng.Acf, *CodCng.Ener) == 0) {
            ...
            *Ftyp = 2;
            ...
            temp = g723abs_s(g723sub(curQGain, CodCng.IRef));
            if(temp > ThreshGain) {
                *Ftyp = 2;
            }
            else {
  ...
  */
    if(CodCng.PastFtyp == 1) {
        CodCng.CurGain = CodCng.SidGain;
          ...
          CodCng.CurGain = g723extract_h(g723L_add( g723L_mult(CodCng.CurGain,0x7000),
                    g723L_mult(CodCng.SidGain,0x1000) ) ) ;
    }
...
void Update_Acf(Word16 *Acf_sf, Word16 *ShAcf_sf)
{
    ...
    sh1 = g723add(sh1, 14);  /* 2 bits of margin */
    ...
    for(i_subfr=0; i_subfr<SubFrames; i_subfr++) {
        temp = g723sub(sh1, ShAcf_sf[i_subfr]);
        for(i=0; i <= LpcOrder; i++) {
            L_acc0 = g723L_deposit_l(*ptr2++);
            L_acc0 = g723L_shl(L_acc0, temp);  /* shift right if temp<0 */
 //           L_temp[i] = g723L_add(L_temp[i], L_acc0);
			L_ADD(L_temp[i], L_acc0, L_temp[i]);
        }
    }
    /* Normalize */
    temp = g723norm_l(L_temp[0]);
    temp = g723sub(16, temp);
    if(temp < 0) temp = 0;
    for(i=0; i <= LpcOrder; i++) {
        CodCng.Acf[i] = g723extract_l(g723L_shr(L_temp[i],temp));
    }

    CodCng.ShAcf[0] = g723sub(sh1, temp);
...
void ComputePastAvFilter(Word16 *Coeff)
{
    ...
    sh1 = g723add(sh1, 14);     /* 2 bits of margin : NbAvAcf <= 4 */
    ...
    for(i=1; i <= NbAvAcf; i ++) {
        temp = g723sub(sh1, CodCng.ShAcf[i]);
        for(j=0; j <= LpcOrder; j++) {
            L_acc0 = g723L_deposit_l(*ptr_Acf++);
            L_acc0 = g723L_shl(L_acc0, temp); /* shift right if temp<0 */
 //           L_sumAcf[j] = g723L_add(L_sumAcf[j], L_acc0);
			L_ADD(L_sumAcf[j], L_acc0, L_sumAcf[j]);
        }
    }

    /* Normalize */
    temp = g723norm_l(L_sumAcf[0]);
    temp = g723sub(16, temp);
    if(temp < 0) temp = 0;
    Err = g723extract_l(g723L_shr(L_sumAcf[0],temp));
    for(i=1; i<LpcOrderP1; i++) {
        Corr[i-1] = g723extract_l(g723L_shr(L_sumAcf[i],temp));
    }
...
void CalcRC(Word16 *Coeff, Word16 *RC, Word16 *ShRC)
{
    ...
    L_acc = 0L;
    for(j=0; j<LpcOrder; j++) {
        L_acc = g723L_mac(L_acc, Coeff[j], Coeff[j]);
    }
    L_acc = g723L_shr(L_acc, 1);
    L_acc = g723L_add(L_acc, 0x04000000L);  /* 1 << 2 * Lpc_justif. */
    sh1 = g723norm_l(L_acc) - (Word16)2;    /* 1 bit because of x2 in RC[i], i> 0*/
                                /* & 1 bit margin for Itakura distance */
    L_acc = g723L_shl(L_acc, sh1); /* shift right if < 0 */
    RC[0] = g723round(L_acc);

    for(i=1; i<=LpcOrder; i++) {
 //       L_acc = L_mult( (Word16) 0xE000, Coeff[i-1]);   /* - (1 << Lpc_justif.) */
		L_MULT((Word16) 0xE000, Coeff[i-1], L_acc);
        for(j=0; j<LpcOrder-i; j++) {
            L_acc = g723L_mac(L_acc, Coeff[j], Coeff[j+i]);
        }
        L_acc = g723L_shl(L_acc, sh1);
        RC[i] = g723round(L_acc);
    }
    *ShRC = sh1;
    return;
}
...
Flag LpcDiff(Word16 *RC, Word16 ShRC, Word16 *ptrAcf, Word16 alpha)
{
    ...
    L_temp0 = 0L;
    for(i=0; i<=LpcOrder; i++) {
        temp = g723shr(ptrAcf[i], 2);  /* + 2 margin bits */
        L_temp0 = g723L_mac(L_temp0, RC[i], temp);
    }

    temp = g723mult_r(alpha, FracThresh);
    L_temp1 = g723L_add((Word32)temp, (Word32)alpha);
    temp = g723add(ShRC, 9);  /* 9 = Lpc_justif. * 2 - 15 - 2 */
    L_temp1 = g723L_shl(L_temp1, temp);
