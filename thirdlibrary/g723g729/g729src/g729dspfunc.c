...
)
{
  ...
  L_x = g729L_mult(fraction, 32);           /* L_x = fraction<<6           */
  i   = g729extract_h(L_x);                 /* Extract b10-b15 of fraction */
  L_x = g729L_shr(L_x, 1);
  a   = g729extract_l(L_x);                 /* Extract b0-b9   of fraction */
  a   = a & (Word16)0x7fff;

  L_x = g729L_deposit_h(tabpow[i]);          /* tabpow[i] << 16        */
  tmp = g729sub(tabpow[i], tabpow[i+1]);      /* tabpow[i] - tabpow[i+1] */
  L_x = g729L_msu(L_x, tmp, a);             /* L_x -= tmp*a*2        */

  exp = g729sub(30, exponent);
  L_x = g729L_shr_r(L_x, exp);
...
)
{
  ...
  exp = g729norm_l(L_x);
  L_x = g729L_shl(L_x, exp );               /* L_x is normalized */

  *exponent = g729sub(30, exp);

  L_x = g729L_shr(L_x, 9);
  i   = g729extract_h(L_x);                 /* Extract b25-b31 */
  L_x = g729L_shr(L_x, 1);
  a   = g729extract_l(L_x);                 /* Extract b10-b24 of fraction */
  a   = a & (Word16)0x7fff;

  i   = g729sub(i, 32);

  L_y = g729L_deposit_h(tablog[i]);          /* tablog[i] << 16        */
  tmp = g729sub(tablog[i], tablog[i+1]);      /* tablog[i] - tablog[i+1] */
  L_y = g729L_msu(L_y, tmp, a);             /* L_y -= tmp*a*2        */

  *fraction = g729extract_h( L_y);
...
)
{
  ...
  exp = g729norm_l(L_x);
  L_x = g729L_shl(L_x, exp );               /* L_x is normalize */
  ...
  i   = g729extract_h(L_x);                 /* Extract b25-b31 */
  L_x = g729L_shr(L_x, 1);
  a   = g729extract_l(L_x);                 /* Extract b10-b24 */
  a   = a & (Word16)0x7fff;

  i   = g729sub(i, 16);

  L_y = g729L_deposit_h(tabsqr[i]);          /* tabsqr[i] << 16          */
  tmp = g729sub(tabsqr[i], tabsqr[i+1]);      /* tabsqr[i] - tabsqr[i+1])  */
  L_y = g729L_msu(L_y, tmp, a);             /* L_y -=  tmp*a*2         */

  L_y = g729L_shr(L_y, exp);                /* denormalization */
