...
_declspec(dllexport) void Init_Post_Process(void)
{
  y2_hi = 0;
  y2_lo = 0;
  y1_hi = 0;
...
_declspec(dllexport) void Post_Process(
  ...
  Word16 lg)          /* length of signal    */
{
  ...
  for(i=0; i<lg; i++)
  {
     ...
     L_tmp     = g729L_add(L_tmp, Mpy_32_16(y2_hi, y2_lo, a100[2]));
     L_tmp     = g729L_mac(L_tmp, x0, b100[0]);
     L_tmp     = g729L_mac(L_tmp, x1, b100[1]);
     L_tmp     = g729L_mac(L_tmp, x2, b100[2]);
     L_tmp     = g729L_shl(L_tmp, 2);      /* Q29 --> Q31 (Q13 --> Q15) */

     /* Multiplication by two of output speech with saturation. */
     signal[i] = g729round(g729L_shl(L_tmp, 1));
