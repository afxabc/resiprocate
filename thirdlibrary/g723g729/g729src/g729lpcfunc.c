...
)
{
  ...
  for (i = 5; i > 0; i--)
  {
    f1[i] = g729L_add(f1[i], f1[i-1]);        /* f1[i] += f1[i-1]; */
    f2[i] = g729L_sub(f2[i], f2[i-1]);        /* f2[i] -= f2[i-1]; */
  }

  a[0] = 4096;
  for (i = 1, j = 10; i <= 5; i++, j--)
  {
    t0   = g729L_add(f1[i], f2[i]);                 /* f1[i] + f2[i]             */
    a[i] = g729extract_l( g729L_shr_r(t0, 13) );        /* from Q24 to Q12 and * 0.5 */

    t0   = g729L_sub(f1[i], f2[i]);                 /* f1[i] - f2[i]             */
    a[j] = g729extract_l( g729L_shr_r(t0, 13) );        /* from Q24 to Q12 and * 0.5 */
...
static void Get_lsp_pol(Word16 *lsp, Word32 *f)
{
   ...
   *f = g729L_mult(4096, 2048);             /* f[0] = 1.0;             in Q24  */
   f++;
   *f = g729L_msu((Word32)0, *lsp, 512);    /* f[1] =  -2.0 * lsp[0];  in Q24  */
   ...
   for(i=2; i<=5; i++)
   {
     *f = f[-2];
     ...
     for(j=1; j<i; j++, f--)
     {
       ...
       t0 = g729L_shl(t0, 1);
       *f = g729L_add(*f, f[-2]);                /* *f += f[-2]         */
       *f = g729L_sub(*f, t0);                   /* *f -= t0            */
     }
     *f   = g729L_msu(*f, *lsp, 512);            /* *f -= lsp<<9        */
...
)
{
  ...
  for(i=0; i<m; i++)
  {
    ind    = g729shr(lsf[i], 8);               /* ind    = b8-b15 of lsf[i] */
    offset = lsf[i] & (Word16)0x00ff;      /* offset = b0-b7  of lsf[i] */

    /* lsp[i] = table[ind]+ ((table[ind+1]-table[ind])*offset) / 256 */

    L_tmp   = g729L_mult(g729sub(table[ind+1], table[ind]), offset);
    lsp[i] = g729add(table[ind], g729extract_l(g729L_shr(L_tmp, 9)));
  }
  return;
}
...
)
{
  ...
  for(i= m-(Word16)1; i >= 0; i--)
  {
    ...
    while( g729sub(table[ind], lsp[i]) < 0 )
    {
      ind = g729sub(ind,1);
    }
    ...
    L_tmp  = g729L_mult( g729sub(lsp[i], table[ind]) , slope[ind] );
    tmp = g729round(g729L_shl(L_tmp, 3));     /*(lsp[i]-table[ind])*slope[ind])>>12*/
    lsf[i] = g729add(tmp, g729shl(ind, 8));
  }
  return;
}
...
)
{
  ...
  {
/*    freq = abs_s(freq);*/
    freq = g729mult(lsf[i], 20861);          /* 20861: 1.0/(2.0*PI) in Q17 */
    ind    = g729shr(freq, 8);               /* ind    = b8-b15 of freq */
    ...
    L_tmp   = g729L_mult(slope_cos[ind], offset);   /* L_tmp in Q28 */
    lsp[i] = g729add(table2[ind], g729extract_l(g729L_shr(L_tmp, 13)));
...
)
{
  ...
  for(i= m-(Word16)1; i >= 0; i--)
  {
    ...
    while( g729sub(table2[ind], lsp[i]) < 0 )
    {
      ind = g729sub(ind,1);
      if ( ind <= 0 )
        break;
    ...
    offset = g729sub(lsp[i], table2[ind]);

    /* acos(lsp[i])= ind*512 + (slope_acos[ind]*offset >> 11) */

    L_tmp  = g729L_mult( slope_acos[ind], offset );   /* L_tmp in Q28 */
    freq = g729add(g729shl(ind, 9), g729extract_l(g729L_shr(L_tmp, 12)));
    lsf[i] = g729mult(freq, 25736);           /* 25736: 2.0*PI in Q12 */
...
)
{
  ...
  for(i=1; i<m; i++)
  {
    ap[i] = g729round( g729L_mult(a[i], fac) );
    fac   = g729round( g729L_mult(fac, gamma) );
  }
  ap[m] = g729round( g729L_mult(a[m], fac) );
...
)
{
  ...
  for (i = 0; i < M; i++) {
    lsp[i] = g729add(g729shr(lsp_new[i], 1), g729shr(lsp_old[i], 1));
