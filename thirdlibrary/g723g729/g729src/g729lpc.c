...
)
{
  ...
  {
    y[i] = g729mult_r(x[i], hamwindow[i]);
  }
  ...
  *exp_R0 = 1;    
  ...
  do {
    ...
    if(g729Overflow != 0)
    {
      ...
      *exp_R0 = g729add((*exp_R0), 4);
      g729Overflow = 1;
    }
  }while (g729Overflow != 0);

  /* Normalization of r[0] */

  norm = g729norm_l(sum);
  sum  = g729L_shl(sum, norm);
  L_Extract(sum, &r_h[0], &r_l[0]);     /* Put in DPF format (see oper_32b) */
  *exp_R0 = g729sub(*exp_R0, norm);
...
/* K = A[1] = -R[1] / R[0] */
  ...
  t2  = g729L_abs(t1);                      /* abs R[1]         */
  t0  = Div_32(t2, Rh[0], Rl[0]);       /* R[1]/R[0] in Q31 */
  if(t1 > 0) t0= g729L_negate(t0);          /* -R[1]/R[0]       */
  L_Extract(t0, &Kh, &Kl);              /* K in DPF         */
  rc[0] = Kh;
  t0 = g729L_shr(t0,4);                     /* A[1] in Q27      */
  L_Extract(t0, &Ah[1], &Al[1]);        /* A[1] in DPF      */

/*  Alpha = R[0] * (1-K**2) */

  t0 = Mpy_32(Kh ,Kl, Kh, Kl);          /* K*K      in Q31 */
  t0 = g729L_abs(t0);                       /* Some case <0 !! */
  t0 = g729L_sub( (Word32)0x7fffffffL, t0 ); /* 1 - K*K  in Q31 */
  L_Extract(t0, &hi, &lo);              /* DPF format      */
  t0 = Mpy_32(Rh[0] ,Rl[0], hi, lo);    /* Alpha in Q31    */

/* Normalize Alpha */

  alp_exp = g729norm_l(t0);
  t0 = g729L_shl(t0, alp_exp);
 ...
 *--------------------------------------*/
  ...
  for(i= 2; i<=M; i++)
  {
    ...
    t0 = 0;
    for(j=1; j<i; j++)
      t0 = g729L_add(t0, Mpy_32(Rh[j], Rl[j], Ah[i-j], Al[i-j]));

    t0 = g729L_shl(t0,4);                  /* result in Q27 -> convert to Q31 */
                                       /* No overflow possible            */
    t1 = L_Comp(Rh[i],Rl[i]);
    t0 = g729L_add(t0, t1);                /* g729add R[i] in Q31                 */

    /* K = -t0 / Alpha */

    t1 = g729L_abs(t0);
    t2 = Div_32(t1, alp_h, alp_l);     /* abs(t0)/Alpha                   */
    if(t0 > 0) t2= g729L_negate(t2);       /* K =-t0/Alpha                    */
    t2 = g729L_shl(t2, alp_exp);           /* denormalize; compare to Alpha   */
    L_Extract(t2, &Kh, &Kl);           /* K in DPF                        */
    rc[i-1] = Kh;

    /* Test for unstable filter. If unstable keep old A(z) */

    if (g729sub(g729abs_s(Kh), 32750) > 0)
    {
      for(j=0; j<=M; j++)
      {
        A[j] = old_A[j];
    ...
    for(j=1; j<i; j++)
    {
      ...
      t0 = g729L_add(t0, L_Comp(Ah[j], Al[j]));
      L_Extract(t0, &Anh[j], &Anl[j]);
    }
    t2 = g729L_shr(t2, 4);                  /* t2 = K in Q31 ->convert to Q27  */
    ...
    t0 = g729L_shl(t0, j);
    L_Extract(t0, &alp_h, &alp_l);         /* DPF format    */
    alp_exp = g729add(alp_exp, j);             /* Add normalization to alp_exp */
  ...
  *Err = g729shr(alp_h, alp_exp);
  ...
  {
    t0   = L_Comp(Ah[i], Al[i]);
    old_A[i] = A[i] = g729round(g729L_shl(t0, 1));
  }
  old_rc[0] = rc[0];
  old_rc[1] = rc[1];
 ...
 *-------------------------------------------------------------*/
 ...
 {
   g729Overflow = 0;
   t0 = g729L_mult(a[i+1], 16384);          /* x = (a[i+1] + a[M-i]) >> 1        */
   t0 = g729L_mac(t0, a[M-i], 16384);       /*    -> From Q12 to Q11             */
   x  = g729extract_h(t0);
   if ( g729Overflow ) {
     ovf_coef = 1;      }

   g729Overflow = 0;
   f1[i+1] = g729sub(x, f1[i]);    /* f1[i+1] = a[i+1] + a[M-i] - f1[i] */
   if ( g729Overflow ) {
     ovf_coef = 1;      }

   g729Overflow = 0;
   t0 = g729L_mult(a[i+1], 16384);          /* x = (a[i+1] - a[M-i]) >> 1        */
   t0 = g729L_msu(t0, a[M-i], 16384);       /*    -> From Q12 to Q11             */
   x  = g729extract_h(t0);
   if ( g729Overflow ) {
     ovf_coef = 1;      }

   g729Overflow = 0;
   f2[i+1] = g729add(x, f2[i]);    /* f2[i+1] = a[i+1] - a[M-i] + f2[i] */
   if ( g729Overflow ) {
     ovf_coef = 1;      }
 }
 ...
 if ( ovf_coef ) {
   ...
   for (i = 0; i< NC; i++)
   {
     t0 = g729L_mult(a[i+1], 8192);          /* x = (a[i+1] + a[M-i]) >> 1        */
     t0 = g729L_mac(t0, a[M-i], 8192);       /*    -> From Q11 to Q10             */
     x  = g729extract_h(t0);
     f1[i+1] = g729sub(x, f1[i]);    /* f1[i+1] = a[i+1] + a[M-i] - f1[i] */

     t0 = g729L_mult(a[i+1], 8192);          /* x = (a[i+1] - a[M-i]) >> 1        */
     t0 = g729L_msu(t0, a[M-i], 8192);       /*    -> From Q11 to Q10             */
     x  = g729extract_h(t0);
     f2[i+1] = g729add(x, f2[i]);    /* f2[i+1] = a[i+1] - a[M-i] + f2[i] */
   }
 }
 ...
 *-------------------------------------------------------------*/
 ...
 while ( (nf < M) && (j < GRID_POINTS) )
 {
   ...
   if ( L_temp <= (Word32)0)
   {
     ...
     for (i = 0; i < 4; i++)
     {
       xmid = g729add( g729shr(xlow, 1) , g729shr(xhigh, 1)); /* xmid = (xlow + xhigh)/2 */

       ymid = (*pChebps)(xmid,coef,NC);

       L_temp = g729L_mult(ylow,ymid);
       if ( L_temp <= (Word32)0)
       {
         yhigh = ymid;
         xhigh = xmid;
     ...
     *-------------------------------------------------------------*/
     ...
     x   = g729sub(xhigh, xlow);
     y   = g729sub(yhigh, ylow);
     ...
     else
     {
       ...
       exp = g729norm_s(y);
       y   = g729shl(y, exp);
       y   = g729div_s( (Word16)16383, y);
       t0  = g729L_mult(x, y);
       t0  = g729L_shr(t0, g729sub(20, exp) );
       y   = g729extract_l(t0);            /* y= (xhigh-xlow)/(yhigh-ylow) in Q11 */

       if(sign < 0) y = g729negate(y);

       t0   = g729L_mult(ylow, y);                  /* result in Q26 */
       t0   = g729L_shr(t0, 11);                    /* result in Q15 */
       xint = g729sub(xlow, g729extract_l(t0));         /* xint = xlow - ylow*y */
     }
...
static Word16 Chebps_11(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q24. */
  ...
  t0 = g729L_mult(x, 512);                  /* 2*x in Q24          */
  t0 = g729L_mac(t0, f[1], 4096);           /* + f[1] in Q24       */
  ...
  for (i = 2; i<n; i++)
  {
    ...
    t0 = g729L_shl(t0, 1);
    t0 = g729L_mac(t0,b2_h,(Word16)-32768L); /* t0 = 2.0*x*b1 - b2         */
    t0 = g729L_msu(t0, b2_l, 1);
    t0 = g729L_mac(t0, f[i], 4096);         /* t0 = 2.0*x*b1 - b2 + f[i]; */
  ...
  t0 = g729L_mac(t0, b2_h,(Word16)-32768L);  /* t0 = x*b1 - b2          */
  t0 = g729L_msu(t0, b2_l, 1);
  t0 = g729L_mac(t0, f[i], 2048);           /* t0 = x*b1 - b2 + f[i]/2 */

  t0 = g729L_shl(t0, 6);                    /* Q24 to Q30 with saturation */
  cheb = g729extract_h(t0);                 /* Result in Q14              */
...
static Word16 Chebps_10(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q23. */
  ...
  t0 = g729L_mult(x, 256);                  /* 2*x in Q23          */
  t0 = g729L_mac(t0, f[1], 4096);           /* + f[1] in Q23       */
  L_Extract(t0, &b1_h, &b1_l);          /* b1 = 2*x + f[1]     */
  ...
  t0 = g729L_shl(t0, 7);                    /* Q23 to Q30 with saturation */
  cheb = g729extract_h(t0);                 /* Result in Q14              */
