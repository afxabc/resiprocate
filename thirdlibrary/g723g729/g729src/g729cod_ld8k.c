...
_declspec(dllexport) void Init_Coder_ld8k(void)
{
...
_declspec(dllexport) void Coder_ld8k(
 ...
 *------------------------------------------------------------------------*/
  ...
  *----------------------------------------------------------------------*/
  ...
  if ((Vad == 0) && (vad_enable == 1)){
    ...
    for(i_subfr=0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
      ...
      /* update mem_w0 */
      for (i=0; i<L_SUBFR; i++)
        error[i] = g729sub(speech[i_subfr+i], synth[i_subfr+i]);
  ...
  *----------------------------------------------------------------------*/
  ...
  T0_min = g729sub(T_op, 3);
  if (g729sub(T0_min,PIT_MIN)<0) {
    T0_min = PIT_MIN;
  }

  T0_max = g729add(T0_min, 6);
  if (g729sub(T0_max ,PIT_MAX)>0)
  {
     T0_max = PIT_MAX;
     T0_min = g729sub(T0_max, 6);
  }
  ...
  *------------------------------------------------------------------------*/
  ...
  for (i_subfr = 0;  i_subfr < L_FRAME; i_subfr += L_SUBFR)
  {
    ...
    *-----------------------------------------------------------------*/
    ...
    for (i = 0; i < L_SUBFR; i++)
    {
      L_temp = g729L_mult(y1[i], gain_pit);
      L_temp = g729L_shl(L_temp, 1);               /* gain_pit in Q14 */
      xn2[i] = g729sub(xn[i], g729extract_h(L_temp));
    }
    ...
    *-----------------------------------------------------*/
    ...
    exp_g_coeff_cs[0] = g729negate(g_coeff[1]);           /* Q-Format:XXX -> JPN  */
    g_coeff_cs[1]     = g729negate(g_coeff[2]);           /* (xn,y1) -> -2<xn,y1> */
    exp_g_coeff_cs[1] = g729negate(g729add(g_coeff[3], 1));   /* Q-Format:XXX -> JPN  */
    ...
    *------------------------------------------------------*/
    ...
    for (i = 0; i < L_SUBFR;  i++)
    {
      ...
      L_temp = g729L_mult(exc[i+i_subfr], gain_pit);
      L_temp = g729L_mac(L_temp, code[i], gain_code);
    ...
    for (i = L_SUBFR-M, j = 0; i < L_SUBFR; i++, j++)
    {
      mem_err[j] = g729sub(speech[i_subfr+i], synth[i_subfr+i]);
      temp       = g729extract_h(g729L_shl( g729L_mult(y1[i], gain_pit),  1) );
      k          = g729extract_h(g729L_shl( g729L_mult(y2[i], gain_code), 2) );
      mem_w0[j]  = g729sub(xn[i], g729add(temp, k));
    }
...
)
{
      ...
      for(i=0; i<L_SUBFR; i++)
         scaled_y2[i] = g729shr(y2[i], 3);

      /* Compute scalar product <y2[],y2[]> */

      L_acc = 1;                       /* Avoid case of all zeros */
      for(i=0; i<L_SUBFR; i++)
         L_acc = g729L_mac(L_acc, scaled_y2[i], scaled_y2[i]);    /* L_acc:Q19 */

      exp      = g729norm_l(L_acc);
      y2y2     = g729round( g729L_shl(L_acc, exp) );
      exp_y2y2 = g729add(exp, 19-16);                          /* Q[19+exp-16] */
      ...
      L_acc = 1;                       /* Avoid case of all zeros */
      for(i=0; i<L_SUBFR; i++)
         L_acc = g729L_mac(L_acc, xn[i], scaled_y2[i]);           /* L_acc:Q10 */

      exp      = g729norm_l(L_acc);
      xny2     = g729round( g729L_shl(L_acc, exp) );
      exp_xny2 = g729add(exp, 10-16);                          /* Q[10+exp-16] */

      g_coeff[3]     = g729negate(xny2);
      exp_g_coeff[3] = g729sub(exp_xny2,1);                   /* -2<xn,y2> */

      /* Compute scalar product <y1[],y2[]> */

      L_acc = 1;                       /* Avoid case of all zeros */
      for(i=0; i<L_SUBFR; i++)
         L_acc = g729L_mac(L_acc, y1[i], scaled_y2[i]);           /* L_acc:Q10 */

      exp      = g729norm_l(L_acc);
      y1y2     = g729round( g729L_shl(L_acc, exp) );
      exp_y1y2 = g729add(exp, 10-16);                          /* Q[10+exp-16] */

      g_coeff[4]     = y1y2;
      exp_g_coeff[4] = g729sub(exp_y1y2,1);    ;                /* 2<y1,y2> */

      return;
}

_declspec(dllexport) Word16 * GetNewSpeech()
{
	return new_speech;
}