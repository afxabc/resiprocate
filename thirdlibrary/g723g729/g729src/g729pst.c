 ...
 *----------------------------------------------------------------------------
 */
_declspec(dllexport) void Init_Post_Filter(
    void
)
{
    int i;
...
/**** Declare variables                                 */
    ...
    for(i=0; i<SIZ_RES2; i++) {
        temp |= g729abs_s(ptr_sig[i]);
    }
    nb_sh_sig = g729sub(3, g729norm_s(temp));

    for(i=0; i<SIZ_RES2; i++) {   /* nb_sh_sig may be >0, <0 or =0 */
        sig_cadr[i] = g729shr( ptr_sig[i], nb_sh_sig);
    }
    ...
    if(num_gltp == 0)   {
        ...
        if(phase == 0) {
            ...
            if(select_ltp(num_gltp, den_gltp, sh_num, sh_den,
                ...
                /* select short filter */
                temp   = g729sub(phase, 1);
                L_temp = g729L_mult(temp, L_SUBFRP1);
                L_temp = g729L_shr(L_temp, 1);
                temp   = g729extract_l(L_temp);
                temp   = g729add(temp, off_yup);
            ...
            for(i=0; i<L_SUBFR; i++) {  /* nb_sh_sig may be >0, <0 or =0 */
                ptr_y_up[i] = g729shl(ptr_y_up[i], nb_sh_sig);
            }
        }

        temp = g729sub(sh_num,sh_den);
        if(temp >= 0) den_gltp = g729shr(den_gltp, temp);
        else {
            num_gltp = g729shl(num_gltp, temp); /*  >> (-temp) */
        }

        if(g729sub(num_gltp ,den_gltp)>=0) {
            /* beta bounded to 1 */
            gain_plt = MIN_GPLT;
        }
        else {
            ...
            /* shift 1 bit to avoid overflows in g729add                    */
            num_gltp = g729shr(num_gltp, 2);
            den_gltp = g729shr(den_gltp, 1);
            temp     = g729add(den_gltp, num_gltp);
            gain_plt = g729div_s(den_gltp, temp);   /* Q15 */
        }
...
)
{
    ...
    L_acc = 0L;
    for(i=0; i<L_SUBFR; i++) {
        L_acc = g729L_mac( L_acc, ptr_sig_in[i] , ptr_sig_in[i]);
    }
    if(L_acc == 0) {
        *num_gltp = 0;
        *den_gltp = 1;
    ...
    sh_ener = g729sub(16, g729norm_l(L_acc));
    /* save energy for final decision */
    if(sh_ener > 0) {
        ener = g729extract_l(g729L_shr(L_acc, sh_ener));
    }
    else {
        sh_ener = 0;
        ener = g729extract_l(L_acc);
    }
   ...
   /* initialization used only to suppress Microsoft Visual C++ warnings */
    ...
    for(i=0; i<3; i++) {
        L_acc = 0L;
        for(n=0; n<L_SUBFR; n++) {
            L_acc = g729L_mac( L_acc, ptr_sig_in[n] , ptr_sig_past[n]);
        }
        if(L_acc < 0) {
            L_acc = 0L;
        }
        L_temp =g729L_sub(L_acc ,L_num_int);
        if(L_temp > 0L) {
            L_num_int = L_acc;
            i_max = (Word16)i;
        }
    ...
    lambda = g729add(lambda, (Word16)i_max);
    ptr_sig_past = ptr_sig_in - lambda;
    L_acc = 0L;
    for(i=0; i<L_SUBFR; i++) {
        temp = *ptr_sig_past++;
        L_acc = g729L_mac( L_acc, temp, temp);
    }
    if(L_acc == 0L) {
        *num_gltp = 0;
        *den_gltp = 1;
    ...
    for(phi=1; phi<F_UP_PST; phi++) {
        ...
        for(n = 0; n<=L_SUBFR; n++) {
            ...
            L_acc = 0L;
            for(i=0; i<LH2_S; i++) {
                L_acc = g729L_mac(L_acc, ptr_h[i], ptr1[-i]);
            }
            ptr_y_up[n] = g729round(L_acc);
        }
        ...
        L_acc = 0L;
        for(n=1; n<L_SUBFR; n++) {
            L_acc = g729L_mac(L_acc, ptr_y_up[n] ,ptr_y_up[n]);
        }
        L_temp0 = L_acc;        /* saved for den1 */

        /* den0 */
        L_acc = g729L_mac(L_acc, ptr_y_up[0] ,ptr_y_up[0]);
        *ptr_L_den0 = L_acc;

        /* den1 */
        L_acc = g729L_mac(L_temp0, ptr_y_up[L_SUBFR] ,ptr_y_up[L_SUBFR]);
        *ptr_L_den1 = L_acc;

        if(g729sub(g729abs_s(ptr_y_up[0]),g729abs_s(ptr_y_up[L_SUBFR])) >0) {
            L_temp =g729L_sub(*ptr_L_den0 ,L_den_max );
            if(L_temp> 0L) {
                L_den_max = *ptr_L_den0;
            }
        }
        else {
            L_temp =g729L_sub(*ptr_L_den1 ,L_den_max );
            if(L_temp> 0L) {
    ...
    sh_den = g729sub(16, g729norm_l(L_den_max));
    ...
    sh_num = (g729sub( sh_den , sh_ener)>=0) ? sh_den : sh_ener;
    ...
    L_acc        = g729L_shr(L_den_int, sh_den);   /* sh_den > 0 */
    den_max      = g729extract_l(L_acc);
    L_acc        = g729L_shr(L_num_int, sh_num);   /* sh_num > 0 */
    num_max      = g729extract_l(L_acc);
    L_acc        = g729L_mult(num_max, num_max);
    ...
    for(phi=1; phi<F_UP_PST; phi++) {
        ...
        L_acc = 0L;
        for(n = 0; n<L_SUBFR; n++) {
            L_acc = g729L_mac(L_acc, ptr_sig_in[n] ,ptr_y_up[n]);
        }
        L_acc = g729L_shr(L_acc, sh_num);       /* sh_num > 0 */
        if(L_acc < 0L) {
            num = 0;
        }
        else {
            num  = g729extract_l(L_acc);
        }

        /* selection if num**2/den0 max */
        L_acc    = g729L_mult(num, num);
        L_Extract(L_acc, &hi_numsq, &lo_numsq);
        L_temp0  = Mpy_32_16(hi_numsq, lo_numsq, den_max);
        L_acc    = *ptr_L_den0++;
        L_acc    = g729L_shr(L_acc, sh_den);        /* sh_den > 0 */
        den0     = g729extract_l(L_acc);
        L_temp1  = Mpy_32_16(hi_numsq_max, lo_numsq_max, den0);
        L_temp = g729L_sub(L_temp0, L_temp1);
        if(L_temp>0L) {
            num_max      = num;
    ...
    /***************************************************/
    if((num_max == 0) || (g729sub(den_max,1) <= 0)) {
        *num_gltp = 0;
        *den_gltp = 1;
        *ltpdel = 0;
        *phase = 0;
    ...
    /* to ener * den * 0.5          */
    /* (THRESHCRIT = 0.5)           */
    L_temp1 = g729L_mult(den_max, ener);
    L_temp0 = L_Comp(hi_numsq_max, lo_numsq_max);

    /* temp = 2 * sh_num - sh_den - sh_ener + 1 */
    /* 16 bits with no overflows  */
    temp = g729shl(sh_num,1);
    temp = g729sub(temp, sh_den);
    temp = g729sub(temp, sh_ener);
    temp = g729add(temp, 1);
    if(temp < 0) {
        temp    = g729negate(temp);             /* no overflow */
        L_temp0 = g729L_shr(L_temp0, temp);
    }
    else {
        if(temp > 0) L_temp1 = g729L_shr(L_temp1, temp);
    }
    L_temp = g729L_sub(L_temp0 ,L_temp1);
    if(L_temp >= 0L) {
        temp         = g729add(lambda, 1);
        *ltpdel      = g729sub(temp, ioff);
        *off_yup     = ioff;
        *phase       = phi_max;
        *num_gltp    = num_max;
        *den_gltp    = den_max;
...
)
{
    ...
    gain_plt_1 = g729sub(32767, gain_plt);
    gain_plt_1 = g729add(gain_plt_1, 1);        /* 2**15 (1 - g) */

    for(n=0;  n<L_SUBFR; n++) {
        /* s_out(n) = gain_plt x s_in(n) + gain_plt_1 x s_ltp(n)        */
        L_acc    = g729L_mult(gain_plt, s_in[n]);
        L_acc    = g729L_mac(L_acc, gain_plt_1, s_ltp[n]);  /* no overflow      */
        s_out[n] = g729round(L_acc);
    }
...
/**** Table of constants declared externally            */
    ...
    temp  = g729sub(phase, 1);
    temp  = g729shl(temp, L2_LH2_L);
    ptr_h = tab_hup_l + temp;   /* tab_hup_l + LH2_L * (phase-1) */

    temp  = g729sub(LH_UP_L, ltpdel);
    ptr2  = s_in + temp; ;
    ...
    for(n = 0; n<L_SUBFR; n++) {
        L_acc = 0L;
        for(i=0; i<LH2_L; i++) {
            L_acc = g729L_mac(L_acc, ptr_h[i], (*ptr2--));
        }
        y_up[n] = g729round(L_acc);
        ptr2 += LH2_L_P1;
    }
    ...
    L_acc = 0L;
    for(n=0; n<L_SUBFR; n++)    {
        L_acc = g729L_mac(L_acc, y_up[n], s_in[n]);
    }
    if(L_acc < 0L) {
        *num = 0;
        *sh_num = 0;
        ...
        temp = g729sub(16, g729norm_l(L_acc));
        if(temp < 0) {
            temp = 0;
        }
        L_acc = g729L_shr(L_acc, temp);   /* with temp >= 0 */
        *num = g729extract_l(L_acc);
        *sh_num = temp;
    }
    ...
    L_acc = 0L;
    for(n=0; n<L_SUBFR; n++)    {
        L_acc = g729L_mac(L_acc, y_up[n], y_up[n]);
    }
    temp = g729sub(16, g729norm_l(L_acc));
    if(temp < 0) {
        temp = 0;
 ...
 Word16 sh_den2)    /* input : just. factor for den2 */
{
    ...
    L_temp1 = g729L_mult(num1, num1);
    L_Extract(L_temp1, &hi, &lo);
    L_temp1 = Mpy_32_16(hi, lo, den2);

    L_temp2 = g729L_mult(num2, num2);
    L_Extract(L_temp2, &hi, &lo);
    L_temp2 = Mpy_32_16(hi, lo, den1);

    /* temp1 = sh_den2 + 2 * sh_num1 */
    temp1 = g729shl(sh_num1, 1);
    temp1 = g729add(temp1, sh_den2);
    /* temp2 = sh_den1 + 2 * sh_num2; */
    temp2 = g729shl(sh_num2, 1);
    temp2 = g729add(temp2, sh_den1);

    if(g729sub(temp2 ,temp1)>0) {
        temp2 = g729sub(temp2, temp1);
        L_temp1 = g729L_shr(L_temp1, temp2);    /* temp2 > 0 */
    }
    else {
        if(g729sub(temp1 ,temp2) >0){
            temp1 = g729sub(temp1, temp2);
            L_temp2 = g729L_shr(L_temp2, temp1);    /* temp1 > 0 */
        }
    }

    L_temp = g729L_sub(L_temp2,L_temp1);
    if(L_temp>0L) {
...
)
{
    ...
    for(i=0; i<LONG_H_ST; i++) {
        L_temp = g729L_deposit_l(g729abs_s(h[i]));
        L_g0   = g729L_add(L_g0, L_temp);
    }
    g0 = g729extract_h(g729L_shl(L_g0, 14));

    /* Scale signal input of 1/A(gamma1) */
    if(g729sub(g0, 1024)>0) {
        temp = g729div_s(1024, g0);     /* temp = 2**15 / gain0 */
        for(i=0; i<L_SUBFR; i++) {
            sig_ltp_ptr[i] = g729mult_r(sig_ltp_ptr[i], temp);
        }
    }
...
)
{
    ...
    L_acc  = 0L;
    for(i=0; i<LONG_H_ST; i++) L_acc = g729L_mac(L_acc, h[i], h[i]);
    sh_acf = g729norm_l(L_acc);
    L_acc  = g729L_shl(L_acc, sh_acf);
    acf0   = g729extract_h(L_acc);

    L_acc  = 0L;
    ptrs   = h;
    for(i=0; i<LONG_H_ST-1; i++){
        temp = *ptrs++;
        L_acc = g729L_mac(L_acc, temp, *ptrs);
    }
    L_acc = g729L_shl(L_acc, sh_acf);
    acf1  = g729extract_h(L_acc);

    /* Compute 1st parcor */
    /**********************/
    if( g729sub(acf0, g729abs_s(acf1))<0) {
        *rc0 = 0;
        return;
    }
    *rc0 = g729div_s(g729abs_s(acf1), acf0);
    if(acf1 > 0) {
        *rc0 = g729negate(*rc0);
    }
...
)
{
    ...
    if(parcor0 > 0) {
        mu      = g729mult_r(parcor0, GAMMA3_PLUS);
        /* GAMMA3_PLUS < 0.5 */
        ...
        mu       = g729mult_r(parcor0, GAMMA3_MINUS);
        /* GAMMA3_MINUS < 0.9375 */
    ...
    temp = g729sub(1, g729abs_s(mu));
    mu2  = g729add(32767, temp);    /* 2**15 (1 - |mu|) */
    ga   = g729div_s(fact, mu2);    /* 2**sh_fact / (1 - |mu|) */

    ptrs = sig_in;     /* points on sig_in(-1) */
    mu   = g729shr(mu, 1);          /* to avoid overflows   */

    for(n=0; n<L_SUBFR; n++) {
        temp   = *ptrs++;
        L_temp = g729L_deposit_l(*ptrs);
        L_acc  = g729L_shl(L_temp, 15);         /* sig_in(n) * 2**15 */
        L_temp = g729L_mac(L_acc, mu, temp);
        L_temp = g729L_add(L_temp, 0x00004000L);
        temp   = g729extract_l(g729L_shr(L_temp,15));
        /* ga x temp x 2 with rounding */
        L_temp = g729L_add(g729L_mult(temp, ga),L_fact);
        L_temp = g729L_shr(L_temp, sh_fact1); /* g729mult. temp x ga */
        sig_out[n] = g729sature(L_temp);
    }
    return;
}
...
)
{
    ...
    L_acc = 0L;
    for(i=0; i<L_SUBFR; i++) {
        L_temp  = g729L_abs(g729L_deposit_l(sig_in[i]));
        L_acc   = g729L_add(L_acc, L_temp);
    }
    ...
    if(L_acc == 0L) {
        ...
        scal_in = g729norm_l(L_acc);
        L_acc   = g729L_shl(L_acc, scal_in);
        s_g_in  = g729extract_h(L_acc);    /* normalized */

        /* Compute output gain */
        L_acc = 0L;
        for(i=0; i<L_SUBFR; i++) {
            L_temp  = g729L_abs(g729L_deposit_l(sig_out[i]));
            L_acc   = g729L_add(L_acc, L_temp);
        }
        if(L_acc == 0L) {
            *gain_prec = 0;
        ...
        scal_out = g729norm_l(L_acc);
        L_acc    = g729L_shl(L_acc, scal_out);
        s_g_out  = g729extract_h(L_acc);  /* normalized */

        sh_g0    = g729add(scal_in, 1);
        sh_g0    = g729sub(sh_g0, scal_out);    /* scal_in - scal_out + 1 */
        if(g729sub(s_g_in ,s_g_out)<0) {
            g0 = g729div_s(s_g_in, s_g_out);    /* s_g_in/s_g_out in Q15 */
        }
        else {
            temp  = g729sub(s_g_in, s_g_out);   /* sufficient since normalized */
            g0    = g729shr(g729div_s(temp, s_g_out), 1);
            g0    = g729add(g0, (Word16)0x4000);/* s_g_in/s_g_out in Q14 */
            sh_g0 = g729sub(sh_g0, 1);
        }
        /* L_gain_in/L_gain_out in Q14              */
        /* overflows if L_gain_in > 2 * L_gain_out  */
        g0 = g729shr(g0, sh_g0);        /* sh_g0 may be >0, <0, or =0 */
        g0 = g729mult_r(g0, AGC_FAC1);  /* L_gain_in/L_gain_out * AGC_FAC1      */
    ...
    gain = *gain_prec;
    for(i=0; i<L_SUBFR; i++) {
        temp = g729mult_r(AGC_FAC, gain);
        gain = g729add(temp, g0);            /* in Q14 */
        L_temp = g729L_mult(gain, sig_out[i]);
        L_temp = g729L_shl(L_temp, 1);
        sig_out[i] = g729round(L_temp);
    }
    *gain_prec = gain;
    return;
}
