...
)
{
  ...
  for (k=0; k<M; k++) {
    LsfInt[k] = g729shl(LsfInt[k], 1);
    LsfNew[k] = g729shl(LsfNew[k], 1);
  }
  ...
  for (i=0; i<2; i++) {
    ...
    cur_rc = g729abs_s(r_c[i]);
    cur_rc = g729shr(cur_rc, 4);

    if (g729sub(cur_rc ,SEG1)<= 0) {
        LarNew[i] = cur_rc;
    }
    else {
      if (g729sub(cur_rc,SEG2)<= 0) {
        cur_rc = g729shr(cur_rc, 1);
        L_temp = g729L_mult(cur_rc, A1);
        L_temp = g729L_sub(L_temp, L_B1);
        L_temp = g729L_shr(L_temp, 11);
        LarNew[i] = g729extract_l(L_temp);
      }
      else {
        if (g729sub(cur_rc ,SEG3)<= 0) {
          cur_rc = g729shr(cur_rc, 1);
          L_temp = g729L_mult(cur_rc, A2);
          L_temp = g729L_sub(L_temp, L_B2);
          L_temp = g729L_shr(L_temp, 11);
          LarNew[i] = g729extract_l(L_temp);
        }
        else {
          cur_rc = g729shr(cur_rc, 1);
          L_temp = g729L_mult(cur_rc, A3);
          L_temp = g729L_sub(L_temp, L_B3);
          L_temp = g729L_shr(L_temp, 11);
          LarNew[i] = g729extract_l(L_temp);
        }
      }
  ...
  temp = g729add(LarNew[0], LarOld[0]);
  Lar[0] = g729shr(temp, 1);
  LarOld[0] = LarNew[0];
  temp = g729add(LarNew[1], LarOld[1]);
  Lar[1] = g729shr(temp, 1);
  LarOld[1] = LarNew[1];
  ...
  for (k=0; k<2; k++) { /* LOOP : gamma2 for 1st to 2nd subframes */
      ...
      if (smooth != 0) {
        if ((g729sub(CritLar0,THRESH_L1)<0)&&( g729sub(CritLar1,THRESH_H1)>0)) {
            smooth = 0;
        }
      }
      else {
        if ( (g729sub(CritLar0 ,THRESH_L2)>0) || (g729sub(CritLar1,THRESH_H2) <0) ) {
            smooth = 1;
        }
    ...
    if (smooth == 0) {
      ...
      d_min = g729sub(Lsf[1], Lsf[0]);
      for (i=1; i<M-1; i++) {
        temp = g729sub(Lsf[i+1],Lsf[i]);
        if (g729sub(temp,d_min)<0) {
            d_min = temp;
        }
      }
      temp = g729mult(ALPHA, d_min);
      temp = g729sub(BETA, temp);
      temp = g729shl(temp, 5);
      gamma2[k] = temp;

      if (g729sub(gamma2[k] , GAMMA2_0_H)>0) {
        gamma2[k] = GAMMA2_0_H;
      }
      if (g729sub(gamma2[k] ,GAMMA2_0_L)<0) {
        gamma2[k] = GAMMA2_0_L;
      }
