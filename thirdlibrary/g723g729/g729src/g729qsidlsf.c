                ...
                )
{
  ...
  if (lsf[0] < L_LIMIT)
    lsf[0] = L_LIMIT;
  for (i=0 ; i < M-1 ; i++)
    if (g729sub(lsf[i+1], lsf[i]) < 2*GAP3) 
      lsf[i+1] = g729add(lsf[i], 2*GAP3);
  if (lsf[M-1] > M_LIMIT)
    lsf[M-1] = M_LIMIT;
  if (lsf[M-1] < lsf[M-2]) 
    lsf[M-2] = g729sub(lsf[M-1], GAP3);
...
)
{
  ...
  *Pptr = ptr_back[0][ptr];
  ...
  for (i=0; i<M/2; i++)
    qlsf[i] = g729add(qlsf[i], lspcb2[PtrTab_2[0][cluster[1]]][i]);
  for (i=M/2; i<M; i++)
    qlsf[i] = g729add(qlsf[i], lspcb2[PtrTab_2[1][cluster[1]]][i]);
...
)
{
  ...
  for (p=0; p<J; p++)
    for (m=0; m<MQ; m++){
      acc0 = 0;
      for (l=0; l<M; l++){
        tmp = g729sub(d_data[p*M+l], lspcb1[PtrTab[m]][l]);
        acc0 = g729L_mac(acc0, tmp, tmp);
      }
      sum[p*MQ+m] = g729extract_h(acc0);
      sum[p*MQ+m] = g729mult(sum[p*MQ+m], Mp[p]);
    }
  ...
  for (q=0; q<K; q++){
    for (l=0; l<M; l++)
      new_d_data[q*M+l] = g729sub(d_data[min_indx_p[q]*M+l], 
...
)
{
  ...
  for (p=0; p<J; p++)
    for (m=0; m<MQ; m++){
      acc0 = 0;
      for (l=0; l<M/2; l++){
        tmp1 = g729extract_h(g729L_shl(g729L_mult(noise_fg_sum[ptr_prd[p]][l], 
                                      noise_fg_sum[ptr_prd[p]][l]), 2));
        tmp1 = g729mult(tmp1, weight[l]);
        tmp2 = g729sub(d_data[p*M+l], lspcb2[PtrTab[0][m]][l]);
        tmp1 = g729extract_h(g729L_shl(g729L_mult(tmp1, tmp2), 3));
        acc0 = g729L_mac(acc0, tmp1, tmp2);
      }
      ...
      for (l=M/2; l<M; l++){
        tmp1 = g729extract_h(g729L_shl(g729L_mult(noise_fg_sum[ptr_prd[p]][l], 
                                      noise_fg_sum[ptr_prd[p]][l]), 2));
        tmp1 = g729mult(tmp1, weight[l]);
        tmp2 = g729sub(d_data[p*M+l], lspcb2[PtrTab[1][m]][l]);
        tmp1 = g729extract_h(g729L_shl(g729L_mult(tmp1, tmp2), 3));
        acc0 = g729L_mac(acc0, tmp1, tmp2);
      }
