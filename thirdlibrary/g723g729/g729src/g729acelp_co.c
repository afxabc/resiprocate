...
)
{
  ...
  *-----------------------------------------------------------------*/
  ...
  sharp = g729shl(pitch_sharp, 1);          /* From Q14 to Q15 */
  if (g729sub(T0, L_SUBFR)<0)
     for (i = T0; i < L_SUBFR; i++){    /* h[i] += pitch_sharp*h[i-T0] */
       h[i] = g729add(h[i], g729mult(h[i-T0], sharp));
     }
  ...
  *-----------------------------------------------------------------*/
  ...
  if(g729sub(T0 ,L_SUBFR) <0)
     for (i = T0; i < L_SUBFR; i++) {  /* code[i] += pitch_sharp*code[i-T0] */
       code[i] = g729add(code[i], g729mult(code[i-T0], sharp));
     }
...
)
{
   ...
   for (i = 0; i < L_SUBFR; i++)
   {
     s = 0;
     for (j = i; j <  L_SUBFR; j++)
       s = g729L_mac(s, X[j], h[j-i]);

     y32[i] = s;

     s = g729L_abs(s);
     L_temp =g729L_sub(s,max);
     if(L_temp>0L) {
        max = s;
     }
   }
   ...
   for(i=0; i<L_SUBFR; i++) {
     D[i] = g729extract_l( g729L_shr(y32[i], j) );
   }
...
)
{
 ...
 /* Scaling h[] for maximum precision */
  ...
  cor = 0;
  for(i=0; i<L_SUBFR; i++)
    cor = g729L_mac(cor, H[i], H[i]);

  L_tmp = g729L_sub(g729extract_h(cor),32000);
  if(L_tmp>0L )
  {
    for(i=0; i<L_SUBFR; i++) {
      h[i] = g729shr(H[i], 1);}
  ...
  *------------------------------------------------------------*/
  ...
  for(i=0;  i<NB_POS; i++)
  {
    cor = g729L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
    *p4-- = g729extract_h(cor);

    cor = g729L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
    *p3-- = g729extract_h(cor);

    cor = g729L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
    *p2-- = g729extract_h(cor);

    cor = g729L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
    *p1-- = g729extract_h(cor);

    cor = g729L_mac(cor, *ptr_h1, *ptr_h1); ptr_h1++;
    *p0-- = g729extract_h(cor);
  }
  ...
  *-----------------------------------------------------------------*/
  ...
  for(k=0; k<NB_POS; k++) {
          ...
          for(i=k+(Word16)1; i<NB_POS; i++ ) {
                  ...
                  cor = g729L_mac(cor, *ptr_h1, *ptr_h2); ptr_h1++; ptr_h2++;
                  cor = g729L_mac(cor, *ptr_h1, *ptr_h2); ptr_h1++; ptr_h2++;
                  *p3 = g729extract_h(cor);
...
)
{
  ...
  *-------------------------------------------------------------------*/
   ...
   {
     if (g729sub(Dn[i]  , max0) > 0){ max0 = Dn[i];   }
     if (g729sub(Dn[i+1], max1) > 0){ max1 = Dn[i+1]; }
     if (g729sub(Dn[i+2], max2) > 0){ max2 = Dn[i+2]; }
   }
   max0 = g729add(max0, max1);
   max0 = g729add(max0, max2);
   ...
   {
     L32 = g729L_mac(L32, Dn[i], 1);
     L32 = g729L_mac(L32, Dn[i+1], 1);
     L32 = g729L_mac(L32, Dn[i+2], 1);
   }
   average =g729extract_l( g729L_shr(L32, 4));   /* 1/8 of sum */

   /* thres = average + (max0-average)*THRESHFCB; */

   thres = g729sub(max0, average);
   thres = g729mult(thres, THRESHFCB);
   thres = g729add(thres, average);
  ...
  ptr_ri0i4 = rri0i4;
   ...
   for(i0=0; i0<L_SUBFR; i0+=STEP)
   {
     ...
     {
       *ptr_ri0i1 = g729mult(*ptr_ri0i1, g729mult(p_sign[i0], p_sign[i1]));
        ptr_ri0i1++;
       *ptr_ri0i2 = g729mult(*ptr_ri0i2, g729mult(p_sign[i0], p_sign[i1+1]));
        ptr_ri0i2++;
       *ptr_ri0i3 = g729mult(*ptr_ri0i3, g729mult(p_sign[i0], p_sign[i1+2]));
        ptr_ri0i3++;
       *ptr_ri0i4 = g729mult(*ptr_ri0i4, g729mult(p_sign[i0], p_sign[i1+3]));
        ptr_ri0i4++;
   ...
   for(i1=1; i1<L_SUBFR; i1+=STEP)
   {
      ...
      {
        *ptr_ri1i2 = g729mult(*ptr_ri1i2, g729mult(p_sign[i1], p_sign[i2]));
         ptr_ri1i2++;
        *ptr_ri1i3 = g729mult(*ptr_ri1i3, g729mult(p_sign[i1], p_sign[i2+1]));
         ptr_ri1i3++;
        *ptr_ri1i4 = g729mult(*ptr_ri1i4, g729mult(p_sign[i1], p_sign[i2+2]));
         ptr_ri1i4++;
   ...
   for(i2=2; i2<L_SUBFR; i2+=STEP)
   {
      ...
      {
        *ptr_ri2i3 = g729mult(*ptr_ri2i3, g729mult(p_sign[i2], p_sign[i3]));
         ptr_ri2i3++;
        *ptr_ri2i4 = g729mult(*ptr_ri2i4, g729mult(p_sign[i2], p_sign[i3+1]));
         ptr_ri2i4++;
 ...
 for (i0 = 0; i0 < L_SUBFR; i0 += STEP)        /* first pulse loop  */
 {
   ...
   {
     ps1  = g729add(ps0, Dn[i1]);

     /* alp1 = alp0 + *ptr_ri1i1++ + 2.0 * ( *ptr_ri0i1++); */

     alp1 = g729L_mult(alp0, 1);
     alp1 = g729L_mac(alp1, *ptr_ri1i1++, 1);
     alp1 = g729L_mac(alp1, *ptr_ri0i1++, 2);
     ...
     for (i2 = 2; i2 < L_SUBFR; i2 += STEP)    /* third pulse loop */
      {
       ps2  = g729add(ps1, Dn[i2]);

       /* alp2 = alp1 + *ptr_ri2i2++ + 2.0 * (*ptr_ri0i2++ + *ptr_ri1i2++); */

       alp2 = g729L_mac(alp1, *ptr_ri2i2++, 1);
       alp2 = g729L_mac(alp2, *ptr_ri0i2++, 2);
       alp2 = g729L_mac(alp2, *ptr_ri1i2++, 2);

       /* Test threshold */

       if ( g729sub(ps2, thres) > 0)
       {
         ...
         {
           ps3 = g729add(ps2, Dn[i3]);

           /* alp3 = alp2 + *ptr_ri3i3++                                */
           /*       + 2.0*( *ptr_ri0i3++ + *ptr_ri1i3++ + *ptr_ri2i3++); */

           alp3 = g729L_mac(alp2, *ptr_ri3i3++, 1);
           alp3 = g729L_mac(alp3, *ptr_ri0i3++, 2);
           alp3 = g729L_mac(alp3, *ptr_ri1i3++, 2);
           alp3 = g729L_mac(alp3, *ptr_ri2i3++, 2);
           alp  = g729extract_l(g729L_shr(alp3, 5));

           ps3c = g729mult(ps3, ps3);
           L_temp = g729L_mult(ps3c, alpha);
           L_temp = g729L_msu(L_temp, psc, alp);
           if( L_temp > 0L )
           {
             psc = ps3c;
             alpha = alp;
         ...
         for (i3 = 4; i3 < L_SUBFR; i3 += STEP)      /* 4th pulse loop */
         {
           ...
           alp3 = g729L_mac(alp2, *ptr_ri4i4++, 1);
           alp3 = g729L_mac(alp3, *ptr_ri0i4++, 2);
           alp3 = g729L_mac(alp3, *ptr_ri1i4++, 2);
           alp3 = g729L_mac(alp3, *ptr_ri2i4++, 2);
           alp  = g729extract_l(g729L_shr(alp3, 5));
...
end_search:
 ...
 cod[ip0] = g729shr(i0, 2);         /* From Q15 to Q13 */
 cod[ip1] = g729shr(i1, 2);
 cod[ip2] = g729shr(i2, 2);
 cod[ip3] = g729shr(i3, 2);
 ...
 i = 0;
 if(i0 > 0) i = g729add(i, 1);
 if(i1 > 0) i = g729add(i, 2);
 if(i2 > 0) i = g729add(i, 4);
 if(i3 > 0) i = g729add(i, 8);
 *sign = i;

 ip0 = g729mult(ip0, 6554);         /* ip0/5 */
 ip1 = g729mult(ip1, 6554);         /* ip1/5 */
 ip2 = g729mult(ip2, 6554);         /* ip2/5 */
 i   = g729mult(ip3, 6554);         /* ip3/5 */
 j   = g729add(i, g729shl(i, 2));       /* j = i*5 */
 j   = g729sub(ip3, g729add(j, 3));     /* j= ip3%5 -3 */
 ip3 = g729add(g729shl(i, 1), j);

 i = g729add(ip0, g729shl(ip1, 3));
 i = g729add(i  , g729shl(ip2, 6));
 i = g729add(i  , g729shl(ip3, 9));
