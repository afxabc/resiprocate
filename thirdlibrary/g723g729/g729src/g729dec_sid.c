...
**
*/
_declspec(dllexport) void Init_Dec_cng(void)
{
...
void Init_lsfq_noise(void)
{
  ...
  for (i=0; i<4; i++)
    for (j=0; j<M; j++){
      acc0 = g729L_mult(fg[0][i][j], 19660);
      acc0 = g729L_mac(acc0, fg[1][i][j], 13107);
      noise_fg[1][i][j] = g729extract_h(acc0);
    }
}
                     ...
                     )
{
  ...
  for (i=0; i<M/2; i++)
    tmpbuf[i] = g729add(tmpbuf[i], lspcb2[PtrTab_2[0][index[2]]][i]);
  for (i=M/2; i<M; i++)
    tmpbuf[i] = g729add(tmpbuf[i], lspcb2[PtrTab_2[1][index[2]]][i]);

  /* guarantee minimum distance of 0.0012 (~10 in Q13) between tmpbuf[j] 
     and tmpbuf[j+1] */
  for (j=1; j<M; j++){
    acc0 = g729L_mult(tmpbuf[j-1], 16384);
    acc0 = g729L_mac(acc0, tmpbuf[j], -16384);
    acc0 = g729L_mac(acc0, 10, 16384);
    k = g729extract_h(acc0);

    if (k > 0){
      tmpbuf[j-1] = g729sub(tmpbuf[j-1], k);
      tmpbuf[j] = g729add(tmpbuf[j], k);
    }
  }
