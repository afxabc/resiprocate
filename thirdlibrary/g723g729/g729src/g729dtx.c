...
)
{
  ...
  if(pastVad != 0) {
    ...
    nb_ener = g729add(nb_ener, 1);
    if(g729sub(nb_ener, NB_GAIN) > 0) nb_ener = NB_GAIN;
    Qua_Sidgain(ener, sh_ener, nb_ener, &energyq, &cur_igain);
    ...
    temp = g729abs_s(g729sub(prev_energy, energyq));
    temp = g729sub(temp, 2);
    if (temp > 0) flag_chang = 1;
      
    count_fr0 = g729add(count_fr0, 1);
    if(g729sub(count_fr0, FR_SID_MIN) < 0) {
  ...
  if(pastVad != 0) {
    ...
    cur_gain = g729mult_r(cur_gain, A_GAIN0);
    cur_gain = g729add(cur_gain, g729mult_r(sid_gain, A_GAIN1));
  }
...
)
{
  ...
  /* Save current Acf */
  sh_Acf[0] = g729negate(g729add(16, exp_r));
  for(i=0; i<MP1; i++) {
    Acf[i] = r_h[i];
  }

  fr_cur = g729add(fr_cur, 1);
  if(g729sub(fr_cur, NB_CURACF) == 0) {
    fr_cur = 0;
    if(Vad != 0) {
      Update_sumAcf();
    }
...
static void Calc_RCoeff(Word16 *Coeff, Word16 *RCoeff, Word16 *sh_RCoeff)
{
  ...
  L_acc = 0L;
  for(j=0; j <= M; j++) {
    L_acc = g729L_mac(L_acc, Coeff[j], Coeff[j]);
  }
  
  /* Compute exponent RCoeff */
  sh1 = g729norm_l(L_acc);
  L_acc = g729L_shl(L_acc, sh1);
  RCoeff[0] = g729round(L_acc);
  
  /* RCoeff[i] = SUM(j=0->M-i) Coeff[j] * Coeff[j+i] */
  for(i=1; i<=M; i++) {
    L_acc = 0L;
    for(j=0; j<=M-i; j++) {
      L_acc = g729L_mac(L_acc, Coeff[j], Coeff[j+i]);
    }
    L_acc = g729L_shl(L_acc, sh1);
    RCoeff[i] = g729round(L_acc);
  }
  *sh_RCoeff = sh1;
  return;
}
                                        ...
                                        Word16 alpha, Word16 FracThresh)
{
  ...
  do {
    g729Overflow = 0;
    temp1 = g729shr(RCoeff[0], sh[0]);
    temp2 = g729shr(acf[0], sh[1]);
    L_temp0 = g729L_shr(g729L_mult(temp1, temp2),1);
    for(i=1; i <= M; i++) {
      temp1 = g729shr(RCoeff[i], sh[0]);
      temp2 = g729shr(acf[i], sh[1]);
      L_temp0 = g729L_mac(L_temp0, temp1, temp2);
    }
    if(g729Overflow != 0) {
      sh[(int)ind] = g729add(sh[(int)ind], 1);
      ind = g729sub(1, ind);
    }
    else flag = 1;
  } while (flag == 0);
  ...
  temp1 = g729mult_r(alpha, FracThresh);
  L_temp1 = g729L_add(g729L_deposit_l(temp1), g729L_deposit_l(alpha));
  temp1 = g729add(sh_RCoeff, 9);  /* 9 = Lpc_justif. * 2 - 16 + 1 */
  temp2 = g729add(sh[0], sh[1]);
  temp1 = g729sub(temp1, temp2);
  L_temp1 = g729L_shl(L_temp1, temp1);
  
  L_temp0 = g729L_sub(L_temp0, L_temp1);
                         ...
                         Word16 *sum, Word16 *sh_sum, Word16 nb)
{
  ...
  sh0 = sh_acf[0];
  for(i=1; i<nb; i++) {
    if(g729sub(sh_acf[i], sh0) < 0) sh0 = sh_acf[i];
  }
  sh0 = g729add(sh0, 14);           /* 2 bits of margin */
  ...
  ptr1 = acf;
  for(i=0; i<nb; i++) {
    temp = g729sub(sh0, sh_acf[i]);
    for(j=0; j<MP1; j++) {
      L_temp = g729L_deposit_l(*ptr1++);
      L_temp = g729L_shl(L_temp, temp); /* shift right if temp<0 */
      L_tab[j] = g729L_add(L_tab[j], L_temp);
    }
  } 
  temp = g729norm_l(L_tab[0]);
  for(i=0; i<=M; i++) {
    sum[i] = g729extract_h(g729L_shl(L_tab[i], temp));
  }
  temp = g729sub(temp, 16);
  *sh_sum = g729add(sh0, temp);
  return;
}
