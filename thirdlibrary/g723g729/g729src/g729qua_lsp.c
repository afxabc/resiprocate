...
)
{
  ...
  for(mode = 0; mode<MODE; mode++) {
    ...
    for( j = 0 ; j < NC ; j++ )
      buf[j] = g729add( lspcb1[cand_cur][j], lspcb2[index][j] );
  ...
  code_ana[0] = g729shl( mode_index,NC0_B ) | cand[mode_index];
  code_ana[1] = g729shl( tindex1[mode_index],NC1_B ) | tindex2[mode_index];
...
)
{
  ...
  *cand = 0;
  ...
  for ( i = 0 ; i < NC0 ; i++ ) {
    L_tmp = 0;
    for ( j = 0 ; j < M ; j++ ) {
      tmp = g729sub(rbuf[j], lspcb1[i][j]);
      L_tmp = g729L_mac( L_tmp, tmp, tmp );
    }

    L_temp = g729L_sub(L_tmp,L_dmin);
    if (  L_temp< 0L) {
      L_dmin = L_tmp;
      *cand = i;
    }
...
)
{
  ...
  *index = 0;
  ...
  for ( k1 = 0 ; k1 < NC1 ; k1++ ) {
    L_dist = 0;
    for ( j = 0 ; j < NC ; j++ ) {
      tmp = g729sub(buf[j], lspcb2[k1][j]);
      tmp2 = g729mult( wegt[j], tmp );
      L_dist = g729L_mac( L_dist, tmp2, tmp );
    }

    L_temp =g729L_sub(L_dist,L_dmin);
    if ( L_temp <0L ) {
      L_dmin = L_dist;
      *index = k1;
    }
...
)
{
  ...
  *L_tdist = 0;
  for ( j = 0 ; j < M ; j++ ) {
    /* tmp = (buf - rbuf)*fg_sum */
    tmp = g729sub( buf[j], rbuf[j] );
    tmp = g729mult( tmp, fg_sum[j] );

    /* *L_tdist += wegt * tmp * tmp */
    L_acc = g729L_mult( wegt[j], tmp );
    tmp2 = g729extract_h( g729L_shl( L_acc, 4 ) );
    *L_tdist = g729L_mac( *L_tdist, tmp2, tmp );
  }
...
)
{
    Word32 L_temp;
  *mode_index = 0;
  L_temp =g729L_sub(L_tdist[1] ,L_tdist[0]);
  if (  L_temp<0L){
    *mode_index = 1;
  }
  return;
...
)
{
  ...
  buf[0] = g729sub( flsp[1], (PI04+8192) );           /* 8192:1.0(Q13) */

  for ( i = 1 ; i < M-1 ; i++ ) {
    tmp = g729sub( flsp[i+1], flsp[i-1] );
    buf[i] = g729sub( tmp, 8192 );
  }

  buf[M-1] = g729sub( (PI92-8192), flsp[M-2] );
  ...
  for ( i = 0 ; i < M ; i++ ) {
    if ( buf[i] > 0 ){
      ...
      L_acc = g729L_mult( buf[i], buf[i] );           /* L_acc in Q27 */
      tmp = g729extract_h( g729L_shl( L_acc, 2 ) );       /* tmp in Q13 */

      L_acc = g729L_mult( tmp, CONST10 );             /* L_acc in Q25 */
      tmp = g729extract_h( g729L_shl( L_acc, 2 ) );       /* tmp in Q11 */

      wegt[i] = g729add( tmp, 2048 );                 /* wegt in Q11 */
    }
  }

  /* */
  L_acc = g729L_mult( wegt[4], CONST12 );             /* L_acc in Q26 */
  wegt[4] = g729extract_h( g729L_shl( L_acc, 1 ) );       /* wegt in Q11 */

  L_acc = g729L_mult( wegt[5], CONST12 );             /* L_acc in Q26 */
  wegt[5] = g729extract_h( g729L_shl( L_acc, 1 ) );       /* wegt in Q11 */

  /* wegt: Q11 -> normalized */
  tmp = 0;
  for ( i = 0; i < M; i++ ) {
    if ( g729sub(wegt[i], tmp) > 0 ) {
      tmp = wegt[i];
    }
  }
  ...
  sft = g729norm_s(tmp);
  for ( i = 0; i < M; i++ ) {
    wegt[i] = g729shl(wegt[i], sft);                  /* wegt in Q(11+sft) */
  }
