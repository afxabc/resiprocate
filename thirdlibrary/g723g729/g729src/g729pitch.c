...
)
{
   ...
   *--------------------------------------------------------*/
   ...
   for(i= -pit_max; i< L_frame; i++)
     t0 = g729L_mac(t0, signal[i], signal[i]);

  /*--------------------------------------------------------*
   * Scaling of input signal.                               *
   *                                                        *
   *   if g729Overflow        -> scal_sig[i] = signal[i]>>3     *
   ...
   *--------------------------------------------------------*/
   ...
   {
     for(i=-pit_max; i<L_frame; i++)
       scal_sig[i] = g729shr(signal[i], 3);
   }
   else {
     L_temp = g729L_sub(t0, (Word32)1048576L);
     if ( L_temp < (Word32)0 )  /* if (t0 < 2^20) */
     {
        for(i=-pit_max; i<L_frame; i++)
          scal_sig[i] = g729shl(signal[i], 3);
     }
     else
     {
       for(i=-pit_max; i<L_frame; i++)
   ...
   *--------------------------------------------------------------------*/
   ...
   i = g729sub(j, 1); j = g729shl(pit_min, 1);
  ...
  if( g729sub(g729mult(max1, THRESHPIT), max2)  < 0)
  {
    max1 = max2;
    p_max1 = p_max2;
  }

  if( g729sub(g729mult(max1, THRESHPIT), max3)  < 0)
  {
    p_max1 = p_max3;
  }
  ...
  Word16 *cor_max)     /* output: normalized correlation of selected lag     */
{
  ...
  for (i = lag_max; i >= lag_min; i--)
  {
    ...
    for (j=0; j<L_frame; j++, p++, p1++)
      t0 = g729L_mac(t0, *p, *p1);

    L_temp = g729L_sub(t0,max);
    if (L_temp >= 0L)
    {
      max    = t0;
      p_max = i;
...
)
{
  ...
  t_min = g729sub(t0_min, L_INTER4);
  t_max = g729add(t0_max, L_INTER4);
...
{
  Word16 i,j,k;
  Word16 corr_h, corr_l, norm_h, g729norm_l;
  Word32 s, L_temp;
  ...
  for(j=0; j<L_subfr; j++)
    scaled_excf[j] = g729shr(excf[j], 2);

  /* Compute energy of excf[] for danger of overflow */

  s = 0;
  for (j = 0; j < L_subfr; j++)
    s = g729L_mac(s, excf[j], excf[j]);

  L_temp = g729L_sub(s, 67108864L);
  ...
  for (i = t_min; i <= t_max; i++)
  {
    ...
    s = 0;
    for (j = 0; j < L_subfr; j++)
      s = g729L_mac(s, s_excf[j], s_excf[j]);

    s = Inv_sqrt(s);                     /* Result in Q30 */
    L_Extract(s, &norm_h, &g729norm_l);

    /* Compute correlation between xn[] and excf[] */

    s = 0;
    for (j = 0; j < L_subfr; j++)
      s = g729L_mac(s, xn[j], s_excf[j]);

    L_Extract(s, &corr_h, &corr_l);

    /* Normalize correlation = correlation * (1/sqrt(energy)) */

    s = Mpy_32(corr_h, corr_l, norm_h, g729norm_l);

    corr_norm[i] = g729extract_h(g729L_shl(s, 16));   /* Result is on 16 bits */

    /* modify the filtered excitation excf[] for the next iteration */

    if( g729sub(i, t_max) != 0)
    {
      k=g729sub(k,1);
      for (j = L_subfr-(Word16)1; j > 0; j--)
      {
        s = g729L_mult(exc[k], h[j]);
        s = g729L_shl(s, h_fac);             /* h is in Q(12-scaling) */
        s_excf[j] = g729add(g729extract_h(s), s_excf[j-1]);
      }
      s_excf[0] = g729shr(exc[k], scaling);
    }
  }
  return;
}
...
)
{
   ...
   for(i=0; i<L_subfr; i++)
     s = g729L_mac(s, y1[i], y1[i]);

   if (g729Overflow == 0) {
     exp_yy = g729norm_l(s);
     yy     = g729round( g729L_shl(s, exp_yy) );
   }
   else {
     s = 1;                  /* Avoid case of all zeros */
     for(i=0; i<L_subfr; i++)
       s = g729L_mac(s, scaled_y1[i], scaled_y1[i]);
     exp_yy = g729norm_l(s);
     yy     = g729round( g729L_shl(s, exp_yy) );
     exp_yy = g729sub(exp_yy, 4);
   }
   ...
   s = 0;
   for(i=0; i<L_subfr; i++)
     s = g729L_mac(s, xn[i], y1[i]);

   if (g729Overflow == 0) {
     exp_xy = g729norm_l(s);
     xy     = g729round( g729L_shl(s, exp_xy) );
   }
   else {
     s = 0;
     for(i=0; i<L_subfr; i++)
       s = g729L_mac(s, xn[i], scaled_y1[i]);
     exp_xy = g729norm_l(s);
     xy     = g729round( g729L_shl(s, exp_xy) );
     exp_xy = g729sub(exp_xy, 2);
   }

   g_coeff[0] = yy;
   g_coeff[1] = g729sub(15, exp_yy);
   g_coeff[2] = xy;
   g_coeff[3] = g729sub(15, exp_xy);
   ...
   xy = g729shr(xy, 1);             /* Be sure xy < yy */
   gain = g729div_s( xy, yy);

   i = g729sub(exp_xy, exp_yy);
   gain = g729shr(gain, i);         /* saturation if > 1.99 in Q14 */

   /* if(gain >1.2) gain = 1.2  in Q14 */

   if( g729sub(gain, 19661) > 0)
   {
     gain = 19661;
   }
...
)
{
  ...
  if (pit_flag == 0)   /* if 1st subframe */
  {
    ...
    if (g729sub(T0, 85) <= 0)
    {
      ...
      i = g729add(g729add(T0, T0), T0);
      index = g729add(g729sub(i, 58), T0_frac);
    }
    else {
      index = g729add(T0, 112);
    }

    /* find T0_min and T0_max for second (or fourth) subframe */

    *T0_min = g729sub(T0, 5);
    if (g729sub(*T0_min, pit_min) < 0)
    {
      *T0_min = pit_min;
    }

    *T0_max = g729add(*T0_min, 9);
    if (g729sub(*T0_max, pit_max) > 0)
    {
      *T0_max = pit_max;
      *T0_min = g729sub(*T0_max, 9);
    }
  }
  else      /* if second subframe */
  {
    ...
    /* index = i*3 + 2 + t0_frac;     */
    i = g729sub(T0, *T0_min);
    i = g729add(g729add(i, i), i);
    index = g729add(g729add(i, 2), T0_frac);
  }
...
)
{
  ...
  x2 = &x[1];
  c1 = &inter_3[frac];
  c2 = &inter_3[g729sub(UP_SAMP,frac)];

  s = 0;
  for(i=0, k=0; i< L_INTER4; i++, k+=UP_SAMP)
  {
    s = g729L_mac(s, x1[-i], c1[k]);
    s = g729L_mac(s, x2[i],  c2[k]);
  }
