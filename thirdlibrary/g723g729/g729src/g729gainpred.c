...
)
{
   ...
   *-------------------------------*/
   ...
   L_tmp = 0;
   for(i=0; i<L_subfr; i++)
     L_tmp = g729L_mac(L_tmp, code[i], code[i]);
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = Mpy_32_16(exp, frac, -24660);   /* Q0 Q15 Q13 -> ^Q14     */
                                           ...
                                           /* lo:Q15+Q13-15+1        */
                                           /* -24660[Q13]=-3.0103    */
   L_tmp = g729L_mac(L_tmp, 32588, 32);        /* 32588*32[Q14]=127.298  */
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = g729L_shl(L_tmp, 10);                      /* From Q14 to Q24 */
   for(i=0; i<4; i++)
     L_tmp = g729L_mac(L_tmp, pred[i], past_qua_en[i]); /* Q13*Q10 ->Q24 */

   *gcode0 = g729extract_h(L_tmp);                    /* From Q24 to Q8  */
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = g729L_mult(*gcode0, 5439);       /* *0.166 in Q15, result in Q24*/
   L_tmp = g729L_shr(L_tmp, 8);             /* From Q24 to Q16             */
   L_Extract(L_tmp, &exp, &frac);       /* Extract exponent of gcode0  */

   *gcode0 = g729extract_l(Pow2(14, frac)); /* Put 14 as exponent so that  */
                                        /* output of Pow2() will be:   */
                                        /* 16768 < Pow2() <= 32767     */
   *exp_gcode0 = g729sub(14,exp);
}
...
)
{
   ...
   *----------------------------------------------------------------------*/
   ...
   L_acc = L_Comp( g729sub(exp,13), frac);         /* L_acc:Q16           */
   tmp = g729extract_h( g729L_shl( L_acc,13 ) );       /* tmp:Q13           */
   past_qua_en[0] = g729mult( tmp, 24660 );        /* past_qua_en[]:Q10 */
}
...
)
{
   ...
   L_tmp = 0;                                                    /* Q10 */
   for(i=0; i<4; i++)
      L_tmp = g729L_add( L_tmp, g729L_deposit_l( past_qua_en[i] ) );
   av_pred_en = g729extract_l( g729L_shr( L_tmp, 2 ) );
   av_pred_en = g729sub( av_pred_en, 4096 );                          /* Q10 */

   if( g729sub(av_pred_en, -14336) < 0 ){
